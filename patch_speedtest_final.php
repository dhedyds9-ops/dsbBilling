...
<div class="p-4 sm:p-6 min-h-[calc(100vh-4rem)] flex flex-col items-center pb-24" 
    ...
    <div class="w-full max-w-sm mt-auto">
        <button type="button" @click="window.startDsSpeedTest(\$data)" x-bind:disabled="isTesting"
...
<script>
    if (typeof window.startDsSpeedTest === 'undefined') {
        window.measureDsPing = async function(url, count = 5) {
            const times = [];
        ...
        window.loadDsManifestTargets = async function() {
            try {
        ...
        window.startDsSpeedTest = async function(ctx) {
            ...
            try {
                const dynamicTargets = await window.loadDsManifestTargets();
                ...
                const pingResult = await window.measureDsPing(pingTarget, 5);
...
echo "Refactored SpeedTest to use global JS.\n";
?>
