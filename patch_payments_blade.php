<?php
$file = 'D:/dsBilling/resources/views/livewire/reseller-portal/billing/payments.blade.php';
...
$tdSearch = <<<PHP
                            <td class="px-6 py-3">
                                <div class="font-medium text-slate-800 dark:text-slate-200">{{ \$payment->customer->name ?? '-' }}</div>
                            </td>
PHP;
...
$tdReplace = <<<PHP
                            ...
                            <td class="px-6 py-3">
                                @php
                                    \$sp = \$payment->customer ? \$payment->customer->customerServices()->with('serviceProfile')->first() : null;
                                @endphp
