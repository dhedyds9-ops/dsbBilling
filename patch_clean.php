...
<div class="p-4 sm:p-6 min-h-[calc(100vh-4rem)] flex flex-col items-center pb-24" 
    ...
    <div class="w-full max-w-sm mt-auto">
        <button type="button" @click="startSpeedTest" x-bind:disabled="isTesting"
    ...
    @script
    <script>
        Alpine.data('speedTestLogic', () => ({
            // State is already defined in x-data on the parent div, but we can extend it or just use the Alpine context
        }));

        // Alternatively, since we just need to modify the parent x-data, we can attach the function to the Alpine scope.
    </script>
    @endscript
