...
class RunDiagnosticsJob implements ShouldQueue
{
    ...
    public function __construct(public int $deviceId, public string $diagnosticType, public array $options, public ?int $taskId = null)
    {
    }
    ...
    public function handle(): void
    {
        ...
        // Actual diagnostics via GenieACS would go here

        if ($this->taskId && isset($task)) {
            $task->update([
                'status' => 'completed',
                'completed_at' => now(),
                'result' => ['diagnostic' => $this->diagnosticType, 'status' => 'success'],
            ]);
        }
    }
}
