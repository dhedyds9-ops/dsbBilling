...
<div class="overflow-x-auto rounded-xl border border-slate-200">
    <table class="w-full {{ $compact ? 'text-sm' : 'text-base' }}">
        ...
        <thead>
            <tr class="bg-slate-50">
                @foreach ($columns as $column)
                    <th
                        class="{{ $compact ? 'px-4 py-2' : 'px-6 py-3' }} text-left text-xs font-semibold text-slate-600 uppercase tracking-wider {{ ($sortable && ($column['sortable'] ?? true)) ? 'cursor-pointer hover:text-slate-900' : '' }}"
        ...
        <tbody class="divide-y divide-slate-200">
            @if ($loading)
                @for ($i = 0; $i < 5; $i++)
                    <tr class="{{ $striped ? ($i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50') : ($hover ? 'hover:bg-slate-50' : '') }}">
                        @foreach ($columns as $column)
            ...
            @else
                @foreach ($data as $index => $row)
                    <tr class="{{ $striped ? ($index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50') : ($hover ? 'hover:bg-slate-50' : '') }}">
                        @foreach ($columns as $column)
                            <td class="{{ $compact ? 'px-4 py-2' : 'px-6 py-4' }} text-sm text-slate-700">
