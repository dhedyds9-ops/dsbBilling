...
<div class="overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700">
    <table class="w-full {{ $compact ? 'text-sm' : 'text-base' }}">
        ...
        <thead>
            <tr class="bg-slate-50 dark:bg-slate-900/50">
                @foreach ($columns as $column)
                    <th
                        class="{{ $compact ? 'px-4 py-2' : 'px-6 py-3' }} text-left text-xs font-semibold text-slate-600 uppercase tracking-wider {{ ($sortable && ($column['sortable'] ?? true)) ? 'cursor-pointer hover:text-slate-900 dark:text-slate-100' : '' }}"
        ...
        <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
            @if ($loading)
                @for ($i = 0; $i < 5; $i++)
                    <tr class="{{ $striped ? ($i % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-900/50/50') : ($hover ? 'hover:bg-slate-50 dark:bg-slate-900/50' : '') }}">
                        @foreach ($columns as $column)
            ...
            @elseif (count($data) === 0)
                <tr>
                    <td colspan="{{ count($columns) }}" class="{{ $compact ? 'px-4 py-8' : 'px-6 py-12' }}">
            ...
            @else
                @foreach ($data as $index => $row)
                    <tr class="{{ $striped ? ($index % 2 === 0 ? 'bg-white dark:bg-slate-800' : 'bg-slate-50 dark:bg-slate-900/50/50') : ($hover ? 'hover:bg-slate-50 dark:bg-slate-900/50' : '') }}">
                        @foreach ($columns as $column)
