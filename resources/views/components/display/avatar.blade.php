...
<div class="relative inline-block" {{ $attributes }}>
    ...
    @else
        <div class="{{ $avatarClasses }} {{ $roundedClass }} bg-slate-200 flex items-center justify-center text-slate-500 ring-2 ring-white">
    ...
    @if ($status)
        <span class="absolute bottom-0 right-0 block {{ $rounded ? 'rounded-full' : 'rounded' }} ring-2 ring-white {{ $statusColors[$status] }} w-3 h-3"></span>
    @endif
</div>
