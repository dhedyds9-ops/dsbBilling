        ...
        <div class="max-h-80 overflow-y-auto">
            @if(count($notifications) > 0)
                @foreach($notifications as $notification)
                    <div wire:key="{{ $notification['id'] }}" class="px-4 py-3 border-b border-slate-100 hover:bg-slate-50 transition-colors cursor-pointer {{ !$notification['read'] ? 'bg-primary-50' : '' }}">
                        <div class="flex items-start gap-3">
                            <div class="mt-0.5 flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center {{ $notification['type'] === 'warning' ? 'bg-yellow-100 text-yellow-600' : ($notification['type'] === 'success' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-blue-600') }}">
