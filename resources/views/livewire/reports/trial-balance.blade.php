        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-8">
                <div class="text-right">
                    ...
                    <p class="text-xl font-bold text-slate-900 dark:text-slate-100">Rp {{ number_format($totals['debit'], 0, ',', '.') }}</p>
                </div>
                <div class="text-right">
                    <p class="text-sm text-slate-500 dark:text-slate-400">Total Kredit</p>
                    <p class="text-xl font-bold text-slate-900 dark:text-slate-100">Rp {{ number_format($totals['credit'], 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </x-base.card>
                    ...
                    @foreach($trialBalance as $section)
                        ...
                        <tr class="bg-slate-100 dark:bg-slate-800">
                            <td colspan="4" class="px-6 py-3">
                                <span class="font-semibold text-slate-900 dark:text-slate-100">{{ $section['label'] }}</span>
                            </td>
                        </tr>
                        ...
                        @foreach($section['items'] as $item)
                            <tr class="hover:bg-slate-50 dark:bg-slate-900/50">
                                <td class="px-6 py-3 text-sm text-slate-600 dark:text-slate-400">{{ $item['code'] }}</td>
                                <td class="px-6 py-3 text-sm text-slate-900 dark:text-slate-100">{{ $item['name'] }}</td>
                        ...
                        <tr class="bg-slate-50 dark:bg-slate-900/50 font-semibold">
                            <td colspan="2" class="px-6 py-3 text-sm text-slate-900 dark:text-slate-100">Total {{ $section['label'] }}</td>
                            <td class="px-6 py-3 text-sm text-slate-900 dark:text-slate-100 text-right">
