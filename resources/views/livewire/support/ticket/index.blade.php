                            ...
                            @forelse ($cards as $t)
                                <div class="p-2 rounded-md border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800/50 hover:shadow-sm cursor-pointer" wire:click="rowDetail({{ $t->id }})">
                                    ...
                                    <div class="mt-1.5 flex items-center justify-between text-[11px]">
                                        <span class="text-slate-500 truncate max-w-[60%]">{{ $t->customer?->name ?? '-' }}</span>
                                        @if ($t->assignedTo)
                    ...
                    <div class="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex items-start justify-between">
                        <div>
                            <div class="flex items-center gap-2">
                                <span class="font-mono text-xs text-slate-500">#{{ $this->timelineData['ticket']->id }}</span>
                            ...
                            <div class="text-xs text-slate-500 mt-0.5">{{ $this->timelineData['ticket']->customer?->name ?? '-' }} · {{ $this->timelineData['ticket']->category }} · Priority <span class="font-medium">{{ $this->timelineData['ticket']->priority }}</span></div>
                        </div>
                    ...
                    @forelse ($pageRows as $t)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40">
                            ...
                            <td class="px-3 py-2 font-mono text-xs">
                                <span class="text-slate-500">#{{ $t->id }}</span>
                            ...
                            <td class="px-3 py-2 text-xs text-slate-500">{{ \Illuminate\Support\Carbon::parse($t->updated_at)->diffForHumans() }}</td>
                            <td class="px-3 py-2 text-right whitespace-nowrap">
                                <div class="inline-flex items-center gap-0.5">
                                    <button wire:click="rowDetail({{ $t->id }})" class="p-1 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded dark:hover:bg-blue-900/30" title="Detail/Timeline">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </button>
                                    <button wire:click="rowEdit({{ $t->id }})" class="p-1 text-slate-500 hover:text-blue-600 hover:bg-blue-50 rounded dark:hover:bg-blue-900/30" title="Edit">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>
                                    </button>
                                    <button wire:click="rowAssign({{ $t->id }})" class="p-1 text-slate-500 hover:text-amber-600 hover:bg-amber-50 rounded dark:hover:bg-amber-900/30" title="Assign">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    </button>
                                    @if (!in_array($t->status, ['resolved','closed']))
                                        <button wire:click="rowResolve({{ $t->id }})" class="p-1 text-slate-500 hover:text-emerald-600 hover:bg-emerald-50 rounded dark:hover:bg-emerald-900/30" title="Resolve">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        </button>
                                    @endif
                                    @if ($t->status !== 'closed')
                                        <button wire:click="rowClose({{ $t->id }})" class="p-1 text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded dark:hover:bg-slate-700" title="Close">
                                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg>
                                        </button>
                                    @endif
                                    <button wire:click="rowDelete({{ $t->id }})" class="p-1 text-slate-500 hover:text-red-600 hover:bg-red-50 rounded dark:hover:bg-red-900/30" title="Delete">
    ...
    @if ($this->showBulkAssign)
        <div x-data="{ show: true }" x-show="show" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
            <div x-show="show" class="w-full max-w-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl">
                <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-700"><h3 class="font-semibold">Bulk Assign Teknisi</h3><p class="text-xs text-slate-500 mt-0.5">{{ count($this->selected) }} ticket terpilih</p></div>
