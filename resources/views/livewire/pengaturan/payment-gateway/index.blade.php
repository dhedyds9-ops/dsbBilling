    ...
    <form class="xl:col-span-2 space-y-4 text-sm">
            ...
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                @foreach(['bank_transfer'=>'Bank Transfer','credit_card'=>'Credit Card','ewallet'=>'e-Wallet','qris'=>'QRIS','direct_debit'=>'Direct Debit','store'=>'Indomaret/Alfamart','cardless_credit'=>'Cardless (Kredivo/Akulaku)'] as $k=>$l)
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300 p-2 rounded bg-slate-50 dark:bg-slate-700/40"><input type="checkbox" wire:model="midtrans.enabled_channels.{{$k}}" class="rounded border-slate-300"> {{ $l }}</label>
                @endforeach
              </div>
            </div>
          </div>
            ...
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-3 gap-1.5">
                @foreach(array_keys($xendit['va_banks']) as $b)
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300 p-1.5 rounded bg-slate-50 dark:bg-slate-700/40"><input type="checkbox" wire:model="xendit.va_banks.{{$b}}" class="rounded border-slate-300"> {{ $b }}</label>
                @endforeach
              </div>
            </div>
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-3 gap-1.5">
                @foreach(array_keys($xendit['ewallet_providers']) as $p)
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300 p-1.5 rounded bg-slate-50 dark:bg-slate-700/40"><input type="checkbox" wire:model="xendit.ewallet_providers.{{$p}}" class="rounded border-slate-300"> {{ $p }}</label>
                @endforeach
              </div>
            </div>
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-3 gap-1.5">
                @foreach(array_keys($xendit['retail_providers']) as $p)
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300 p-1.5 rounded bg-slate-50 dark:bg-slate-700/40"><input type="checkbox" wire:model="xendit.retail_providers.{{$p}}" class="rounded border-slate-300"> {{ $p }}</label>
                @endforeach
              </div>
            </div>
          </div>
            ...
            <div class="md:col-span-2">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-1.5">
                @foreach(array_keys($tripay['enabled_channels']) as $p)
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300 p-1.5 rounded bg-slate-50 dark:bg-slate-700/40"><input type="checkbox" wire:model="tripay.enabled_channels.{{$p}}" class="rounded border-slate-300"> {{ $p }}</label>
                @endforeach
              </div>
            </div>
          </div>
      ...
      @elseif($activeTab === 'manual')
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
          ...
          <div class="p-4 grid grid-cols-1 gap-2">
            @foreach($manualBank['accounts'] as $i => $acc)
              <div class="grid grid-cols-12 gap-2 items-center p-2 rounded bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700">
                <div class="col-span-3"><input type="text" wire:model="manualBank.accounts.{{$i}}.bank_name" placeholder="Nama Bank" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100"></div>
                <div class="col-span-3"><input type="text" wire:model="manualBank.accounts.{{$i}}.account_no" placeholder="No. Rekening" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono"></div>
                ...
                <div class="col-span-1 flex items-center gap-1 justify-end">
                  <label class="inline-flex text-xs"><input type="checkbox" wire:model="manualBank.accounts.{{$i}}.enabled" class="rounded border-slate-300"></label>
                  <button type="button" wire:click="removeBankAccount({{$i}})" class="p-1 text-slate-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
              </div>
            @endforeach
          </div>
      ...
      @elseif($activeTab === 'ewallet')
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
          ...
          <div class="p-4 grid grid-cols-1 gap-2">
            @foreach($ewalletManual['providers'] as $i => $p)
              <div class="grid grid-cols-12 gap-2 items-center p-2 rounded bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700">
                <div class="col-span-3"><input type="text" wire:model="ewalletManual.providers.{{$i}}.name" placeholder="GoPay / OVO / dll" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100"></div>
                <div class="col-span-3"><input type="text" wire:model="ewalletManual.providers.{{$i}}.number" placeholder="No. HP terdaftar" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono"></div>
                <div class="col-span-5"><input type="text" wire:model="ewalletManual.providers.{{$i}}.holder" placeholder="Atas Nama" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100"></div>
                <div class="col-span-1 flex items-center gap-1 justify-end">
                  <label class="inline-flex text-xs"><input type="checkbox" wire:model="ewalletManual.providers.{{$i}}.enabled" class="rounded border-slate-300"></label>
                  <button type="button" wire:click="removeEwallet({{$i}})" class="p-1 text-slate-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </div>
              </div>
            @endforeach
          </div>
