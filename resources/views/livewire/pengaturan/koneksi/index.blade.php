<div>
  <div class="px-4 py-3 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between flex-wrap gap-3">
    ...
    <div class="flex items-center gap-2">
      @if ($savedStatus)
        ...
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="{{ $savedStatus==='saved' ? 'M5 13l4 4L19 7' : 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z' }}"/></svg>
  ...
  <div class="px-3 py-2 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800">
    <nav class="flex items-center gap-1 text-sm font-medium overflow-x-auto">
      @foreach(['router_api'=>'Router API','radius'=>'Radius Engine','genieacs'=>'TR-069 GenieACS','radius_client'=>'Radius Client'] as $k=>$l)
            ...
            @foreach($routers as $r)
              <div class="px-4 py-3 flex items-center justify-between gap-3 hover:bg-slate-50 dark:hover:bg-slate-900/30">
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-2">
                    <span class="inline-flex w-2 h-2 rounded-full {{ !empty($r['active']) ? 'bg-emerald-500' : 'bg-slate-300 dark:bg-slate-500' }}"></span>
                    <span class="font-semibold text-slate-800 dark:text-slate-100">{{ $r['name'] }}</span>
                    <span class="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 uppercase font-mono">{{ $r['brand'] ?? 'mikrotik' }}</span>
                  </div>
                  <div class="text-xs text-slate-500 dark:text-slate-400 font-mono mt-0.5">{{ $r['ip_address'] ?? $r['host'] }}:{{ $r['api_port'] ?? 8728 }} · NAS: {{ $r['nas_identifier'] ?? '-' }}</div>
                </div>
                <div class="flex items-center gap-1">
                  <button wire:click="testRouterConnection({{ $r['id'] }})" title="Test Koneksi" class="p-1.5 rounded text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:text-slate-400 dark:hover:bg-blue-900/30"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg></button>
                  <button wire:click="selectRouter({{ $r['id'] }})" title="Edit" class="p-1.5 rounded text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:text-slate-400 dark:hover:bg-indigo-900/30"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                  <button wire:click="deleteRouter({{ $r['id'] }})" title="Hapus" class="p-1.5 rounded text-slate-500 hover:text-red-600 hover:bg-red-50 dark:text-slate-400 dark:hover:bg-red-900/30"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6M1 7h22"/></svg></button>
                </div>
              </div>
            @endforeach
            @if(empty($routers))
          ...
          <div class="p-4 space-y-2.5 text-sm">
            ...
            <div class="flex items-center gap-2 pt-1">
              <button wire:click="saveRouter" class="flex-1 px-3 py-1.5 text-xs bg-emerald-600 hover:bg-emerald-700 text-white rounded-md inline-flex items-center justify-center gap-1.5">
                ...
                {{ $formRouter['id'] ?? null ? 'Update' : 'Simpan' }} Router
              </button>
        ...
        <div class="space-y-4">
          <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
            ...
            <div class="grid grid-cols-2 gap-2 text-xs">
              <div class="rounded bg-slate-50 dark:bg-slate-700/40 p-2.5"><div class="text-[10px] text-slate-500 dark:text-slate-400">Auth Endpoint</div><div class="font-mono mt-0.5 text-slate-800 dark:text-slate-100 truncate">{{ $radius['auth_host'] ?? '-' }}:{{ $radius['auth_port'] ?? '1812' }}</div></div>
              <div class="rounded bg-slate-50 dark:bg-slate-700/40 p-2.5"><div class="text-[10px] text-slate-500 dark:text-slate-400">Acct Endpoint</div><div class="font-mono mt-0.5 text-slate-800 dark:text-slate-100 truncate">{{ $radius['acct_host'] ?? '-' }}:{{ $radius['acct_port'] ?? '1813' }}</div></div>
              <div class="rounded bg-slate-50 dark:bg-slate-700/40 p-2.5"><div class="text-[10px] text-slate-500 dark:text-slate-400">CoA/PoD (RFC 5176)</div><div class="font-mono mt-0.5 text-slate-800 dark:text-slate-100 truncate">{{ $radius['coa_host'] ?? '-' }}:{{ $radius['coa_port'] ?? '3799' }}</div></div>
              <div class="rounded bg-slate-50 dark:bg-slate-700/40 p-2.5"><div class="text-[10px] text-slate-500 dark:text-slate-400">Protocol Stack</div><div class="font-mono mt-0.5 text-slate-800 dark:text-slate-100">{{ strtoupper($radius['proto'] ?? 'PAP') }} · {{ $radius['nas_port_type'] ?? 'PPPoE' }}</div></div>
            </div>
  ...
  secret = {{ $radius['shared_secret'] ? '****' : '<shared_secret>' }}
        ...
        <div class="space-y-4">
          <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg p-4">
            ...
            <div class="space-y-1.5 text-xs">
              @foreach([['Huawei','HG8245H5 / EG8141A5 / HG8145V5 / HG6145F'],['FiberHome','HG6145D / AN5506-02-F / HG6543C4'],['ZTE','F609v3/v5.2 / F670L / ZXHN H168N / H268A'],['TP-Link','XPON EPON/GPON family (TD-W9970 etc)'],['MikroTik','RouterOS cAP ac, hAP ac² (CWMP enable via /tr069)'],['TOTOLINK / D-Link','XPON ONT berbasis RTL960x']] as [$v,$m])
                <div class="flex items-start gap-2 py-1 border-b border-slate-100 dark:border-slate-700/60 last:border-0">
                  ...
                  <div>
                    <div class="font-semibold text-slate-800 dark:text-slate-100">{{ $v }}</div>
                    <div class="text-[11px] text-slate-500 dark:text-slate-400">{{ $m }}</div>
                  </div>
                </div>
              @endforeach
            </div>
