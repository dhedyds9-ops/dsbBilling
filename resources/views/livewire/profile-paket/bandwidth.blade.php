<div class="max-w-7xl mx-auto p-3">
    ...
    <x-base.card class="overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-700">
                ...
                <tbody class="bg-white dark:bg-slate-900 divide-y divide-gray-200 dark:divide-slate-700">
                    @forelse($bandwidths as $bw)
                        ...
                        @endphp
                        <tr wire:key="{{ $bw->id }}" class="hover:bg-gray-50 dark:hover:bg-slate-800/30 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-cyan-500 text-white flex items-center justify-center font-bold shadow">
                                        {{ $bw->name ? strtoupper(substr($bw->name, 0, 1)) : 'B' }}
                                    </div>
                                    <div>
                                        <div class="text-sm font-semibold text-gray-900 dark:text-white">{{ $bw->name }}</div>
                                        <div class="text-xs text-gray-400 dark:text-slate-500 font-mono">{{ $bw->code ?? '-' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-4">
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-gray-900 dark:text-white">{{ $dlVal }} {{ $unit }}</div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-gray-900 dark:text-white">{{ $ulVal }} {{ $unit }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-4">
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-purple-700 dark:text-purple-300">{{ $dlBurst }} {{ $unit }}</div>
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <div class="text-sm font-bold text-purple-700 dark:text-purple-300">{{ $ulBurst }} {{ $unit }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-4">
                                    <div>
                                        ...
                                        <div class="text-sm font-semibold text-gray-700 dark:text-slate-300">{{ $dlThr }} {{ $unit }}</div>
                                    </div>
                                    <div class="w-px h-8 bg-gray-200 dark:bg-slate-700"></div>
                                    <div>
                                        <span class="text-[10px] font-semibold uppercase text-orange-600">UL Thr</span>
                                        <div class="text-sm font-semibold text-gray-700 dark:text-slate-300">{{ $ulThr }} {{ $unit }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="inline-flex items-center gap-1">
                                    <button wire:click="openEditModal({{ $bw->id }})" title="Edit" type="button" class="p-2 rounded-lg text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/20">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                    </button>
                                    <button wire:click="delete({{ $bw->id }})" wire:confirm="Yakin hapus bandwidth {{ $bw->name }}?" title="Hapus" type="button" class="p-2 rounded-lg text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">
        ...
        @if(method_exists($bandwidths, 'hasPages') && $bandwidths->hasPages())
            <div class="p-4 border-t border-gray-200 dark:border-slate-700">{{ $bandwidths->links() }}</div>
        @endif
    </x-base.card>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 sm:p-0">
            ...
            <div class="inline-block w-full max-w-2xl p-6 my-8 text-left align-middle transition-all transform bg-white dark:bg-slate-800 shadow-2xl rounded-2xl">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                    ...
                    {{ $editingId ? 'Edit' : 'Tambah' }} Bandwidth Profile
                </h3>
                <div class="space-y-4">
                    <div>
                    ...
                    <div class="bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-200 dark:border-emerald-800 p-4 rounded-xl">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                            <div>
                                ...
                                <div class="font-bold text-gray-900 dark:text-white text-sm">{{ number_format($form['burst_limit_download'] ?? 0, 2) }} {{ $form['unit'] ?? 'Mbps' }}</div>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-slate-400">Upload Burst</span>
                                <div class="font-bold text-gray-900 dark:text-white text-sm">{{ number_format($form['burst_limit_upload'] ?? 0, 2) }} {{ $form['unit'] ?? 'Mbps' }}</div>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-slate-400">Burst Time</span>
                                <div class="font-bold text-gray-900 dark:text-white text-sm">{{ $form['burst_time_download'] ?? 8 }}s</div>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-slate-400">DL Threshold</span>
                                <div class="font-bold text-gray-900 dark:text-white text-sm">{{ number_format($form['burst_threshold_download'] ?? 0, 2) }} {{ $form['unit'] ?? 'Mbps' }}</div>
                            </div>
                            <div>
                                <span class="text-gray-500 dark:text-slate-400">UL Threshold</span>
                                <div class="font-bold text-gray-900 dark:text-white text-sm">{{ number_format($form['burst_threshold_upload'] ?? 0, 2) }} {{ $form['unit'] ?? 'Mbps' }}</div>
                            </div>
                            <div>
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <button wire:click="save" type="button" class="px-5 py-2.5 text-sm font-medium text-white bg-primary-600 hover:bg-primary-700 rounded-lg shadow-sm shadow-primary-500/20">
                        {{ $editingId ? 'Simpan Perubahan' : 'Simpan Bandwidth' }}
                    </button>
                </div>
            </div>
        </div>
