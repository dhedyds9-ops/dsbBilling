    ...
    <div class="space-y-4">
        ...
        <div class="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-700/60 relative overflow-hidden">
            ...
            <div class="flex items-center gap-2">
                @if($invoice->status !== 'paid')
                    <a href="{{ route('customer-portal.billing.invoice-show', $invoice->id) }}" class="flex-1 bg-teal-600 hover:bg-teal-700 text-white text-center py-2.5 rounded-xl text-sm font-semibold transition-colors">
                        Bayar Sekarang
                    </a>
                @endif
                <a href="{{ route('customer-portal.billing.invoice-show', $invoice->id) }}" class="flex-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-center py-2.5 rounded-xl text-sm font-semibold transition-colors">
                    Lihat Detail
                </a>
            </div>
        </div>
