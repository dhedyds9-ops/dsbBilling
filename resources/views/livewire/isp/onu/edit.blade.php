<div class="max-w-7xl mx-auto p-3">
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('isp.onus.show', $onu->id) }}" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            ...
            <div>
                ...
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $onu->name }}</p>
            </div>
        </div>
        ...
        <x-base.card>
            <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select wire:model="customer_service_id" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-900 dark:text-slate-100">
                            ...
                            @foreach($customerServices as $cs)
                                <option value="{{ $cs->id }}">
                                    {{ $cs->customer ? $cs->customer->name : 'Unknown' }} 
                                    (PPPoE: {{ $cs->username }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    <a href="{{ route('isp.onus.show', $onu->id) }}" class="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                        Batal
                    </a>
