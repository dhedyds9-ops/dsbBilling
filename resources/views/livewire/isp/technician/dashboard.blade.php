<div class="space-y-4 p-4">
    ...
    <div class="bg-gradient-to-br from-indigo-600 to-blue-700 text-white p-5 rounded-2xl shadow-lg relative overflow-hidden">
        ...
        <div class="relative z-10">
            ...
            <h1 class="text-2xl font-black mb-0.5">{{ auth()->user()->name }}</h1>
    ...
    <div class="grid grid-cols-3 gap-3">
        <div class="bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm text-center">
            ...
            <p class="text-2xl font-black text-slate-800 dark:text-slate-200">{{ $recentInstallations->where('status', 'running')->count() }}</p>
        ...
        <div class="bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm text-center">
            ...
            <p class="text-2xl font-black text-slate-800 dark:text-slate-200">{{ $recentInstallations->where('status', 'completed')->count() }}</p>
        ...
        <div class="bg-white dark:bg-slate-800 p-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm text-center">
            ...
            <p class="text-2xl font-black text-slate-800 dark:text-slate-200">{{ $recentInstallations->where('status', 'failed')->count() }}</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        ...
        <div class="divide-y divide-slate-100 dark:divide-slate-700/50">
            @forelse($recentInstallations as $pipeline)
                <div class="p-4 flex items-center gap-3">
                    <div class="w-10 h-10 rounded-full shrink-0 {{ $pipeline->status === 'completed' ? 'bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600' : ($pipeline->status === 'failed' ? 'bg-red-100 dark:bg-red-900/50 text-red-600' : 'bg-blue-100 dark:bg-blue-900/50 text-blue-600') }} flex items-center justify-center">
                        <span class="material-symbols-outlined" style="font-size:20px">
                            {{ $pipeline->status === 'completed' ? 'check_circle' : ($pipeline->status === 'failed' ? 'error' : 'sync') }}
                        </span>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-bold text-slate-900 dark:text-slate-100 text-sm truncate">
                            {{ $pipeline->serviceInstance->customerService->customer->name ?? 'Unknown' }}
                        </p>
                        <p class="text-xs text-slate-400 mt-0.5">
                            {{ $pipeline->created_at->format('d/m/Y H:i') }}
                        </p>
                    </div>
