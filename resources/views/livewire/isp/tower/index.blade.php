    ...
    <div class="flex justify-between items-center mb-3">
        ...
        <div x-data="{ open: false }" class="relative inline-block text-left">
            ...
            <div x-show="open" @click.away="open = false" x-transition class="origin-top-right absolute right-0 mt-2 w-64 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                <div class="py-1">
                    <a href="{{ route('isp.towers.create') }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                ...
                <div class="py-1">
                    <button
                        ...
                        class="flex items-center gap-2 px-4 py-2 text-sm w-full text-left @if(empty($selectedTowers)) text-gray-300 cursor-not-allowed @else text-red-600 hover:bg-red-50 @endif">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="py-1">
                    ...
                    <button
                        ...
                        wire:loading.attr="disabled"
                        @if(empty($selectedTowers)) disabled @endif
                        class="flex items-center gap-2 px-4 py-2 text-sm w-full text-left @if(empty($selectedTowers)) text-gray-300 cursor-not-allowed @else text-green-600 hover:bg-green-50 @endif">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($towers as $tower)
                        <tr wire:key="{{ $tower->id }}" class="hover:bg-gray-50 transition-colors @if($tower->trashed()) bg-red-50 @endif">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:model.live="selectedTowers" value="{{ $tower->id }}" class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500" @if($tower->trashed()) disabled @endif />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($tower->trashed())
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">Dihapus</span>
                                @else
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full {{ $tower->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-mono text-sm text-gray-900">{{ $tower->code }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600">{{ $tower->district }}, {{ $tower->city }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600">{{ $tower->height }}m</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div x-data="{ open: false }" class="relative inline-block text-left">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition class="origin-top-right absolute right-0 mt-2 w-48 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                                        @if(!$tower->trashed())
                                            <div class="py-1">
                                                <a href="{{ route('isp.towers.show', $tower->id) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <a href="{{ route('isp.towers.edit', $tower->id) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-xl shadow-xl sm:align-middle">
                    <div class="mb-4">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900 mb-2 text-center">Hapus {{ count($selectedTowers) }} Tower?</h3>
