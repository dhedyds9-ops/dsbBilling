        ...
        <div class="flex items-center gap-4">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900">{{ $odp->name }}</h1>
                        <div class="flex items-center gap-2 mt-1">
                            <span class="px-3 py-1 text-xs font-medium rounded-full 
                                        @if($odp->status === 'active') bg-green-100 text-green-600
                                        @else bg-slate-100 text-slate-600
                                        @endif">
                                        {{ ucfirst($odp->status) }}
                                    </span>
                            <span class="text-sm text-slate-500 font-mono">{{ $odp->code }}</span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="{{ route('isp.odps.edit', $odp->id) }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                                    Edit
                                </a>
            </div>
        </div>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200">
                        <div class="p-4 bg-slate-50 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-slate-900">{{ $odp->splitters->count() }}</p>
                        </div>
                    </div>
                </div>
            </x-base.card>
