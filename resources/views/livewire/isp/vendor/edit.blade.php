<div class="max-w-7xl mx-auto p-3">
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            <a href="{{ route('isp.vendors.show', $vendor->id) }}" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                </svg>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Edit Vendor</h1>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $vendor->name }}</p>
            </div>
        </div>
        ...
        <x-base.card>
            <form wire:submit.prevent="save" class="space-y-6">
                ...
                <div class="flex items-center justify-end gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                    <a href="{{ route('isp.vendors.show', $vendor->id) }}" class="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                        Batal
                    </a>
