<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-3">
            <a href="{{ route('isp.vouchers.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors">
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-slate-900">{{ $stats['available'] }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-slate-900">{{ $stats['used'] }}</p>
                </div>
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                ...
                <tbody class="bg-white divide-y divide-slate-200">
                    @forelse($vouchers as $voucher)
                        <tr class="{{ $voucher->trashed() ? 'bg-slate-50' : '' }}">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:model.live="selectedIds" value="{{ $voucher->id }}" class="rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-slate-900">{{ $voucher->customer?->name ?? '-' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $voucher->status === 'available' ? 'bg-green-100 text-green-800' : ($voucher->status === 'used' ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800') }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">
                                {{ $voucher->expires_at?->translatedFormat('d M Y H:i') ?? '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    @if(!$voucher->trashed())
                                        <button wire:click="delete({{ $voucher->id }})" onclick="return confirm('Yakin ingin menghapus voucher ini?')" class="text-red-600 hover:text-red-900" title="Hapus">
