        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div wire:click="$set('filters.status', 'available')" class="relative overflow-x-auto rounded-xl border border-emerald-200 dark:border-emerald-800/60 shadow-md bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-950/50 dark:to-slate-800 group hover:shadow-lg transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="text-4xl font-black text-emerald-700 dark:text-emerald-300 mb-3">{{ number_format($stats['available'] ?? 0) }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Klik untuk memfilter</div>
                </div>
            </div>
            ...
            <div wire:click="$set('filters.status', 'used')" class="relative overflow-x-auto rounded-xl border border-amber-200 dark:border-amber-800/60 shadow-md bg-gradient-to-br from-amber-50 to-white dark:from-amber-950/50 dark:to-slate-800 group hover:shadow-lg transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="text-4xl font-black text-amber-700 dark:text-amber-300 mb-3">{{ number_format($stats['used'] ?? 0) }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Klik untuk memfilter</div>
                </div>
            </div>
            ...
            <div wire:click="$set('filters.status', 'expired')" class="relative overflow-x-auto rounded-xl border border-red-200 dark:border-red-800/60 shadow-md bg-gradient-to-br from-red-50 to-white dark:from-red-950/50 dark:to-slate-800 group hover:shadow-lg transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="text-4xl font-black text-red-700 dark:text-red-300 mb-3">{{ number_format($stats['expired'] ?? 0) }}</div>
                    <div class="text-xs text-slate-500 dark:text-slate-400">Klik untuk memfilter</div>
                </div>
            </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="flex items-center gap-2 w-full sm:w-auto">
                @if(count($selectedIds) > 0)
                    <button onclick="confirm('Yakin ingin menghapus {{ count($selectedIds) }} voucher?') || event.stopImmediatePropagation()" wire:click="confirmBulkDelete" class="inline-flex items-center px-4 py-2 bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 text-sm font-medium rounded-lg transition-colors">
            ...
            <div class="flex items-center gap-3 w-full sm:w-auto">
                <select wire:model.live="filters.voucher_pool_id" class="pl-3 pr-10 py-2 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer dark:bg-slate-900 dark:text-slate-100">
                    ...
                    @foreach($voucherPools as $pool)
                        <option value="{{ $pool->id }}">vc-{{ $pool->created_at->format('d-m-Y-H:i:s') }}</option>
                    @endforeach
                </select>
                ...
                <table class="w-full text-sm">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                        @forelse($vouchers as $voucher)
                            ...
                            <tr class="{{ $rowBg }}">
                                <td class="px-4 py-3">
                                    <input type="checkbox" wire:model.live="selectedIds" value="{{ $voucher->id }}" class="rounded border-slate-300 dark:border-slate-600 text-indigo-600 dark:text-indigo-400 shadow-sm focus:ring-indigo-500">
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }} font-mono text-xs">
                                    {{ $voucher->id }}
                                </td>
                                <td class="px-4 py-3 {{ $textPrimary }} font-mono text-base tracking-wider">
                                    {{ $voucher->login_method === 'username_password' ? ($voucher->hotspotUser?->username ?? $voucher->code) : $voucher->code }}
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }} font-mono tracking-wider">
                                    {{ $voucher->login_method === 'username_password' ? ($voucher->hotspotUser?->cleartext_password ?? '***') : $voucher->code }}
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }}">
                                    {{ $voucher->serviceProfile?->name ?? '-' }}
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }} font-mono text-xs">
                                    Rp {{ number_format($voucher->serviceProfile?->price ?? 0, 0, ',', '.') }}
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }} text-xs">
                                    {{ $voucher->nasDevice?->name ?? 'Semua Server' }}
                                </td>
                                <td class="px-4 py-3 {{ $textSecondary }} text-xs uppercase">
                                    {{ $voucher->type ?? 'HOTSPOT' }}
                                </td>
                                <td class="px-4 py-3 text-xs font-mono">
                                    @if($voucher->created_at)
                                        <button type="button" wire:click="printSingle({{ $voucher->id }})" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 hover:underline transition-colors text-left font-medium" title="Cetak Batch ini">
                                            vc-{{ $voucher->created_at->format('d-m-Y-H:i:s') }}
                                        </button>
                                    @else
                                        -
                                    @endif
                                ...
                                <td class="px-4 py-3 {{ $textSecondary }}">
                                    {{ $voucher->reseller?->name ?? 'Pusat (HQ)' }}
                                </td>
                                ...
                                <td class="px-4 py-3 text-right whitespace-nowrap">
                                    <div class="flex items-center justify-end gap-1">
                                        <button wire:click="printSingle({{ $voucher->id }})" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400 transition-colors" title="Cetak Voucher & Batch">
                                            <span class="material-symbols-outlined notranslate" translate="no" style="font-size:18px">print</span>
                                        </button>

                                        <button wire:click="delete({{ $voucher->id }})" onclick="confirm('Yakin ingin menghapus voucher ini?') || event.stopImmediatePropagation()" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-red-50 dark:bg-red-900/30 hover:bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400 transition-colors" title="Hapus">
...
<!-- Modal Generate Voucher -->
    <div x-data="{ show: @entangle('showGenerateModal') }" x-show="show" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
                    ...
                    <form wire:submit.prevent="generate">
                        ...
                        <div class="px-6 py-6 space-y-4">
                            ...
                            <div>
                                ...
                                @error('service_profile_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                            </div>

                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <!-- Owner -->
                                <div>
                                    <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Owner / Reseller</label>
                                    <select wire:model="reseller_id" class="w-full border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-2 border outline-none dark:bg-slate-900 dark:text-slate-100">
                                        <option value="">Pusat (HQ)</option>
    ...
    document.addEventListener('livewire:init', () => {
        ...
        Livewire.on('open-print-window', (data) => {
            ...
            Swal.fire({
                ...
                color: isDark ? '#f8fafc' : '#0f172a',
                html: `
                    <form id="printVoucherForm" method="POST" action="{{ route('isp.vouchers.print') }}" target="_blank">
                        @csrf
                        ...
                        <div class="mb-4 text-left">
                            ...
                            <select name="template_id" class="w-full border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 rounded-lg focus:ring-indigo-500 focus:border-indigo-500 p-2 border outline-none dark:bg-slate-900 dark:text-slate-100">
                                ...
                                @foreach(\App\Models\ISP\VoucherTemplate::where('is_active', true)->get() as $tpl)
