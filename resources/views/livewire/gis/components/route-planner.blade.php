            ...
            @if($sourceNodeId)
                <div class="flex items-center justify-between p-2 bg-blue-50 rounded-lg border border-blue-200">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ $sourceNodeName ?? 'Selected' }}</span>
                    </div>
                    <button 
            ...
            @if($targetNodeId)
                <div class="flex items-center justify-between p-2 bg-green-50 rounded-lg border border-green-200">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ $targetNodeName ?? 'Selected' }}</span>
                    </div>
                    <button 
        ...
        <div>
            ...
            <div class="grid grid-cols-3 gap-2">
                <button 
                    ...
                    class="px-3 py-2 text-xs font-medium rounded-lg transition-colors {{ $optimizationType === 'distance' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}"
                >
                    Distance
                </button>
                <button 
                    wire:click="$set('optimizationType', 'latency')"
                    class="px-3 py-2 text-xs font-medium rounded-lg transition-colors {{ $optimizationType === 'latency' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}"
                >
                    Latency
                </button>
                <button 
                    wire:click="$set('optimizationType', 'hops')"
                    class="px-3 py-2 text-xs font-medium rounded-lg transition-colors {{ $optimizationType === 'hops' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }}"
                >
                    Hops
                </button>
            </div>
        ...
        <div class="p-4 space-y-3 max-h-64 overflow-y-auto">
            ...
            <div class="space-y-1">
                ...
                <div class="flex items-center space-x-2 text-xs">
                    <span class="w-5 h-5 flex items-center justify-center rounded-full bg-blue-100 text-blue-600 font-medium">{{ $index + 1 }}</span>
                    <span class="flex-1 text-gray-700">{{ $segment['name'] ?? 'Node ' . ($index + 1) }}</span>
    ...
    <div class="border-t border-gray-200">
        ...
        <div class="p-4 pt-0 space-y-2">
            ...
            <div 
                ...
                class="p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50 {{ $selectedRouteId === $alt['id'] ? 'border-blue-500 bg-blue-50' : '' }}"
            >
                <div class="flex items-center justify-between">
                    <span class="text-sm font-medium">Route {{ $index + 1 }}</span>
                    <span class="text-xs text-gray-500">{{ $alt['distance'] ?? 0 }} km</span>
                </div>
            </div>
            @endforeach
        </div>
