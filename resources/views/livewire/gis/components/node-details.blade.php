                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <span class="px-3 py-1.5 text-sm font-medium rounded bg-{{ $nodeType === 'olt' ? 'blue' : 'green' }}-100 text-{{ $nodeType === 'olt' ? 'blue' : 'green' }}-800">
                                {{ strtoupper($nodeType) }}
                            </span>
                    ...
                    <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-3">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2">
                            <div 
                                class="h-2 rounded-full {{ ($nodeData['utilization'] ?? 0) >= 90 ? 'bg-red-500' : (($nodeData['utilization'] ?? 0) >= 75 ? 'bg-amber-500' : 'bg-green-500') }}"
                                style="width: {{ $nodeData['utilization'] ?? 0 }}%"
                            ></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-1.5">
                            <div 
                                class="h-1.5 rounded-full {{ ($metric['value'] ?? 0) > ($metric['threshold'] ?? 100) ? 'bg-red-500' : 'bg-green-500' }}"
                                style="width: {{ min(100, (($metric['value'] ?? 0) / ($metric['threshold'] ?? 100)) * 100) }}%"
                            ></div>
                        </div>
                    </div>
                    @endforeach
