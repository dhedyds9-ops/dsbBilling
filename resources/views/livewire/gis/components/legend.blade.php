<div class="bg-white rounded-lg shadow-sm border border-gray-200">
    ...
    <div class="flex border-b border-gray-200">
        <button 
            ...
            class="flex-1 px-4 py-2 text-sm font-medium {{ $activeTab === 'layers' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}"
        >
            Layers
        </button>
        <button 
            wire:click="$set('activeTab', 'status')"
            class="flex-1 px-4 py-2 text-sm font-medium {{ $activeTab === 'status' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}"
        >
            Status
        </button>
        <button 
            wire:click="$set('activeTab', 'utilization')"
            class="flex-1 px-4 py-2 text-sm font-medium {{ $activeTab === 'utilization' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-gray-500 hover:text-gray-700' }}"
        >
            Utilization
        </button>
    </div>
            ...
            @case('utilization')
                ...
                <div class="space-y-3">
                    ...
                    <div class="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded">
                        ...
                        <span class="text-xs text-gray-500">{{ $item['range'] }}</span>
                    </div>
                    @endforeach
                </div>
                @break
