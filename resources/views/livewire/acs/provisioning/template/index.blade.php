<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('acs.provisioning.templates.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    @forelse($templates as $template)
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{{ $template->vendor ?? '-' }}</td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{{ $template->model ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 text-xs font-medium rounded-full 
                                    @if($template->status === 'active') bg-green-100 dark:bg-green-900/50 text-green-600
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('acs.provisioning.templates.edit', $template->id) }}" class="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:16px">edit</span>
                                    </a>
                                    <button wire:click="delete({{ $template->id }})" wire:confirm="Yakin ingin menghapus template ini?" class="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
