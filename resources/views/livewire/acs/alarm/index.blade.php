<div>
  ...
  <div class="space-y-5 pb-10">
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-medium uppercase tracking-wider 
                                    @if($alarm->severity === 'critical') bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-400
                                    @elseif($alarm->severity === 'major') bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-400
                                    @elseif($alarm->severity === 'minor') bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400
                            ...
                            <td class="px-4 py-3 text-right">
                                <button wire:click="delete({{ $alarm->id }})" wire:confirm="Yakin ingin menghapus alarm ini?" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 transition-colors" title="Hapus">
        ...
        @if(isset($alarms) && method_exists($alarms, 'hasPages') && $alarms->hasPages())
