<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('crm.quotations.index') }}" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ $quotation->customer_name }}</h1>
            <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-sm text-slate-500 dark:text-slate-400">Total: Rp {{ number_format($quotation->total ?? 0, 0, ',', '.') }}</span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('crm.quotations.edit', $quotation->id) }}" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <x-base.card>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-slate-900 dark:text-slate-100">{{ $quotation->customer_name }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Total Harga</label>
                        <p class="text-slate-900 dark:text-slate-100">Rp {{ number_format($quotation->total ?? 0, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Catatan</label>
                        <p class="text-slate-900 dark:text-slate-100">{{ $quotation->notes ?? '-' }}</p>
                    </div>
                </div>
            </x-base.card>
