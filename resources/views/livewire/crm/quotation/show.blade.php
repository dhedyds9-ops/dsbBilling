<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900">{{ $quotation->customer_name }}</h1>
            <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-sm text-slate-500">Total: Rp {{ number_format($quotation->total ?? 0, 0, ',', '.') }}</span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('crm.quotations.edit', $quotation->id) }}" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <x-base.card>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <p class="text-slate-900">Rp {{ number_format($quotation->total ?? 0, 0, ',', '.') }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Catatan</label>
                        <p class="text-slate-900">{{ $quotation->notes ?? '-' }}</p>
                    </div>
                </div>
            </x-base.card>
