<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('crm.installations.index') }}" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">{{ $installation->customer_name }}</h1>
            <div class="flex items-center gap-2 mt-1">
                <span class="px-3 py-1 text-xs font-medium rounded-full 
                    @if($installation->status === 'scheduled') bg-blue-100 dark:bg-blue-900/50 text-blue-600
                    @elseif($installation->status === 'in_progress') bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600
                    @elseif($installation->status === 'completed') bg-green-100 dark:bg-green-900/50 text-green-600
                    @else bg-red-100 dark:bg-red-900/50 text-red-600
                    @endif
                ">
                    {{ ucfirst(str_replace('_', ' ', $installation->status)) }}
                </span>
                <span class="text-sm text-slate-500 dark:text-slate-400">{{ $installation->installation_date?->format('d/m/Y') }}</span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('crm.installations.edit', $installation->id) }}" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <x-base.card>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-slate-900 dark:text-slate-100">{{ $installation->customer_name }}</p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tanggal Instalasi</label>
                            <p class="text-slate-900 dark:text-slate-100">{{ $installation->installation_date?->format('d/m/Y') ?? '-' }}</p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Teknisi</label>
                            <p class="text-slate-900 dark:text-slate-100">{{ $installation->technician ?? '-' }}</p>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Catatan</label>
                        <p class="text-slate-900 dark:text-slate-100">{{ $installation->notes ?? '-' }}</p>
                    </div>
                </div>
            </x-base.card>
