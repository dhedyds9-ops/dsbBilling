  ...
  <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div class="cursor-pointer relative overflow-x-auto rounded-xl border border-red-200 dark:border-red-800/60 shadow-md bg-gradient-to-br from-red-50 to-white dark:from-red-950/50 dark:to-slate-800 transition-all">
          <div class="p-4 pt-5">
              ...
              <div class="text-2xl font-black text-red-700 dark:text-red-300 mb-1">{{ number_format($this->summary['total'] ?? 0) }}</div>
      ...
      <div class="cursor-pointer relative overflow-x-auto rounded-xl border border-amber-200 dark:border-amber-800/60 shadow-md bg-gradient-to-br from-amber-50 to-white dark:from-amber-950/50 dark:to-slate-800 transition-all">
          <div class="p-4 pt-5">
              ...
              <div class="text-2xl font-black text-amber-700 dark:text-amber-300 mb-1">{{ number_format($this->summary['today'] ?? 0) }}</div>
      ...
      <div class="cursor-pointer relative overflow-x-auto rounded-xl border border-slate-200 dark:border-slate-700 shadow-md bg-gradient-to-br from-slate-50 to-white dark:from-slate-800 dark:to-slate-900 transition-all">
          <div class="p-4 pt-5">
              ...
              <div class="text-xl font-black text-slate-700 dark:text-slate-300 mb-1 truncate">{{ $this->summary['terlama'] ?? '-' }}</div>
          ...
          <div class="flex items-center gap-3 w-full sm:w-auto justify-end">
              @if(count($this->selected) > 0)
                  <div class="flex items-center gap-2 bg-indigo-50 dark:bg-indigo-900/30 px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-indigo-800/50">
                      <span class="text-xs font-medium text-indigo-700 dark:text-indigo-400">{{ count($this->selected) }} terpilih</span>
                      <div class="h-4 w-px bg-indigo-200 dark:bg-indigo-700 mx-1"></div>
                      <button wire:click="handleBulkAction('activate', {{ json_encode($this->selected) }})" class="text-xs font-bold text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 px-2 py-1 hover:bg-emerald-50 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 rounded transition-colors">Aktifkan</button>
                      <button wire:click="handleBulkAction('send-wa', {{ json_encode($this->selected) }})" class="text-xs font-bold text-indigo-600 hover:text-indigo-700 dark:text-indigo-400 px-2 py-1 hover:bg-indigo-50 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 rounded transition-colors">Kirim WA</button>
                  </div>
              @endif
        ...
        @forelse ($rows as $row)
          ...
          <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-900/40 transition-colors">
            ...
            <td class="px-3 py-2 text-slate-700 dark:text-slate-200">
              ...
              @if ($phone)
                <div class="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400">{{ $phone }}</div>
              @endif
            </td>
            <td class="px-3 py-2 text-slate-700 dark:text-slate-200">
              {{ $row->package_name ?? '-' }}
      ...
      <div class="w-full max-w-md bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl">
        ...
        <div class="grid grid-cols-1 gap-3 px-5 py-4">
          ...
          <div>
            ...
            @error('formParams.grace_days') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
          </div>
          <div>
