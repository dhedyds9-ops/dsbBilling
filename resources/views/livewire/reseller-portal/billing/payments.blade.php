                    ...
                    @forelse($payments as $row)
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-300">
                                {{ $row->paid_at ? \Carbon\Carbon::parse($row->paid_at)->format('d M Y') : $row->created_at->format('d M Y') }}<br>
                                <span class="text-xs text-slate-400">{{ $row->paid_at ? \Carbon\Carbon::parse($row->paid_at)->format('H:i') : $row->created_at->format('H:i') }}</span>
                            </td>
