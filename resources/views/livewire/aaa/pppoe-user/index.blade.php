<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">PPPoE User Management</h1>
            <p class="mt-1 text-sm text-slate-500">Kelola semua pengguna PPPoE</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <x-base.card>
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm text-slate-500">Total PPPoE User</p>
                ...
                <tbody class="divide-y divide-slate-200">
                    @forelse($pppoeUsers as $user)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600">{{ $user->ip_address ?? '-' }}</td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    @if($user->customer)
                                        <a href="{{ route('crm.customers.customer360', $user->customer->id) }}" class="p-2 text-slate-500 hover:text-primary-600 rounded-lg hover:bg-slate-100">
                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    ...
                                    @endif
                                    <a href="{{ route('aaa.pppoe-users.edit', $user->id) }}" class="p-2 text-slate-500 hover:text-blue-600 rounded-lg hover:bg-slate-100">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                        </svg>
                                    </a>
                                    <button wire:click="delete({{ $user->id }})" wire:confirm="Yakin ingin menghapus PPPoE user ini?" class="p-2 text-slate-500 hover:text-red-600 rounded-lg hover:bg-slate-100">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
