<div class="flex items-center text-xs">
    ...
    @elseif($attendance && !$attendance->checked_out_at)
        <div class="flex items-center gap-2">
            <span class="text-emerald-400 font-mono font-bold tracking-wider" title="Waktu masuk: {{ $attendance->checked_in_at->format('H:i') }}">
                <i class="bi bi-clock mr-0.5"></i> IN: {{ $attendance->checked_in_at->format('H:i') }}
            </span>
    ...
    @else
        <div class="flex items-center gap-2 px-3 py-1 bg-gray-800 text-gray-400 rounded-full font-mono font-semibold" title="Shift Selesai">
            ...
            <span>Selesai ({{ $attendance->checked_out_at->format('H:i') }})</span>
        </div>
    @endif
</div>
