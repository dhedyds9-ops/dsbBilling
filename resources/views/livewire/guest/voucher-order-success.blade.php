<div class="min-h-screen bg-gray-50 dark:bg-gray-900/50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
    ...
    <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md" wire:poll.3s="checkStatus" x-data="{ isPolling: true }" @stop-polling.window="isPolling = false">
        <div class="bg-white dark:bg-slate-800 py-8 px-4 shadow sm:rounded-lg sm:px-10">
            
            @if ($order->status === \App\Models\VoucherOrder::STATUS_COMPLETED)
                ...
                <div class="bg-gray-50 dark:bg-gray-900/50 rounded-lg p-6 border border-gray-200 dark:border-gray-700 mb-6 relative overflow-hidden">
                    ...
                    <div class="relative z-10 space-y-4">
                        <div class="border-b border-gray-200 dark:border-gray-700 pb-4">
                            ...
                            <p class="font-bold text-gray-900 dark:text-gray-100">{{ $order->service_profile_name }}</p>
                        </div>
                        
                        <div>
                            <p class="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Username</p>
                            <div class="flex items-center">
                                <code class="bg-white dark:bg-slate-800 px-3 py-2 rounded border border-gray-300 dark:border-gray-600 font-mono text-lg text-primary-700 flex-1">{{ $order->voucher_username }}</code>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center">
                                <code class="bg-white dark:bg-slate-800 px-3 py-2 rounded border border-gray-300 dark:border-gray-600 font-mono text-lg text-primary-700 flex-1">{{ $order->voucher_password }}</code>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <p class="text-xs text-center text-gray-500 dark:text-gray-400 mt-6">
                    Kredensial ini juga telah dikirimkan ke WhatsApp Anda: <strong>{{ $order->wa_number }}</strong>
                </p>

            @elseif ($order->status === \App\Models\VoucherOrder::STATUS_FAILED)
                <div class="text-center mb-6">
                ...
                <div class="bg-red-50 dark:bg-red-900/30 p-4 rounded-md border border-red-100 mb-6 text-sm text-red-700 text-center">
                    {{ $order->failure_reason ?? 'Kesalahan sistem saat memproses voucher ke Mikrotik.' }}
                </div>
            ...
            @elseif ($order->status === \App\Models\VoucherOrder::STATUS_PENDING)
                <div class="text-center mb-6">
