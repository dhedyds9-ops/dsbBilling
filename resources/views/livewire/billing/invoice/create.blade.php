<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route('billing.invoices.index') }}" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <div>
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100">Buat Invoice Baru</h1>
            <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Tambahkan invoice baru untuk pelanggan</p>
        </div>
    </div>
        ...
        <form wire:submit.prevent="save" class="space-y-6">
                    ...
                    @foreach($items as $index => $item)
                        <div class="grid grid-cols-12 gap-4 items-end p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                            <div class="col-span-6">
                                ...
                                <input type="text" wire:model="items.{{ $index }}.description" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-900 dark:text-slate-100">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Qty</label>
                                <input type="number" wire:model="items.{{ $index }}.quantity" wire:change="calculateTotal" min="1" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-900 dark:text-slate-100">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Harga Satuan</label>
                                <input type="number" wire:model="items.{{ $index }}.unit_price" wire:change="calculateTotal" min="0" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 dark:bg-slate-900 dark:text-slate-100">
                            </div>
                            <div class="col-span-1">
                                <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Total</label>
                                <input type="text" wire:model="items.{{ $index }}.total" readonly class="w-full px-4 py-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm text-slate-600 dark:text-slate-400 dark:bg-slate-900 dark:text-slate-100">
                            </div>
                            <div class="col-span-1">
                                <button type="button" wire:click="removeItem({{ $index }})" class="p-2 text-red-500 hover:bg-red-50 dark:bg-red-900/30 rounded-lg">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                <div class="text-right">
                    ...
                    <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">Rp {{ number_format($total_amount, 0, ',', '.') }}</p>
                </div>
            </div>

            <div class="flex items-center justify-end gap-4 pt-4">
                <a href="{{ route('billing.invoices.index') }}" class="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                    Batal
                </a>
