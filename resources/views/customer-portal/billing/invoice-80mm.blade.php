...
<body onload="window.print()">
    ...
    <div class="header text-center">
        @if(!empty($company['logo_url']))
            <img src="{{ $company['logo_url'] }}" alt="Logo" style="max-height: 40px; margin-bottom: 4px; filter: grayscale(100%);">
        @endif
        <h1>{{ $company['name'] ?? 'BillingHub' }}</h1>
        <p>{{ $company['address'] ?? '' }}</p>
    ...
    <table>
        ...
        <tr>
            <td>Tanggal</td>
            <td>: {{ $invoice->issue_date ? $invoice->issue_date->format('d/m/Y') : date('d/m/Y') }}</td>
        </tr>
        <tr>
            <td>Pelanggan</td>
    ...
    <table>
        ...
        <tr>
            <td colspan="2" class="pb-2">
                {{ is_array($item) ? ($item['description'] ?? 'Item') : ($item->description ?? 'Item') }}<br>
                <div style="display: flex; justify-content: space-between; margin-top: 2px;">
                    <span>{{ is_array($item) ? ($item['quantity'] ?? 1) : ($item->quantity ?? 1) }} x Rp {{ number_format(is_array($item) ? ($item['unit_price'] ?? 0) : ($item->unit_price ?? 0), 0, ',', '.') }}</span>
                    <span>Rp {{ number_format(is_array($item) ? ($item['subtotal'] ?? 0) : ($item->subtotal ?? $item->total ?? 0), 0, ',', '.') }}</span>
                </div>
            </td>
        </tr>
        @endforeach
    ...
    <table>
        ...
        <tr>
            ...
            <td class="text-right font-bold pt-2 border-t">Rp {{ number_format($invoice->total_amount, 0, ',', '.') }}</td>
        </tr>
        <tr>
            <td>Sudah Dibayar</td>
