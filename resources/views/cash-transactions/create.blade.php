...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2>Tambah {{ $defaultType === 'income' ? 'Pendapatan' : ($defaultType === 'expense' ? 'Pengeluaran' : ($defaultType === 'transfer' ? 'Transfer' : 'Transaksi')) }} Kas</h2>
        <a href="{{ route('cash-transactions.index', ['type' => $defaultType]) }}" class="btn btn-secondary">
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('cash-transactions.store') }}" method="POST" id="transactionForm">
                @csrf
                <div class="mb-3">
                    ...
                    <select class="form-control @error('type') is-invalid @enderror" id="type" name="type" required>
                        ...
                        <option value="transfer" {{ old('type', $defaultType ?? '') === 'transfer' ? 'selected' : '' }}>Transfer</option>
                    </select>
