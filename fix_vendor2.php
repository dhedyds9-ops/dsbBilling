...
$endPos = strpos($content, '<table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">');
...
EOD;
    ...
    // Remove bulk actions
    $bulkOld = '<!-- Bulk Actions -->
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4">';
    $bulkEnd = '        </div>
    </div>';
    $bStart = strpos($content, $bulkOld);
    if($bStart !== false) {
        $bEnd = strpos($content, $bulkEnd, $bStart) + strlen($bulkEnd);
        $content = substr($content, 0, $bStart) . substr($content, $bEnd);
    }
    
    // Remove <x-base.card class="mb-3"> Toolbar
    $tbStart = strpos($content, '<!-- Toolbar -->');
    if($tbStart !== false) {
        $tbEnd = strpos($content, '</x-base.card>', $tbStart) + strlen('</x-base.card>');
        $content = substr($content, 0, $tbStart) . substr($content, $tbEnd);
    }
