<?php
$file = 'D:/dsBilling/resources/views/livewire/customer-portal/self-service/active-sessions.blade.php';
$content = <<<HTML
...
<div class="p-4 sm:p-6 min-h-[calc(100vh-4rem)] pb-24">
    <div class="mb-5">
        ...
        <p class="text-xs text-slate-500 mt-1">Daftar perangkat yang sedang terhubung ke internet saat ini.</p>
    </div>

    @if(\$message)
        <div class="mb-4 p-3 rounded-xl text-sm font-medium \{{ \$messageType === 'success' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100' }}">
            {{ \$message }}
        </div>
    @endif
                ...
                <div class="pl-2">
                    <div class="flex justify-between items-start mb-3">
                        <div class="flex items-center gap-2">
                            <span class="shrink-0 px-2 py-0.5 rounded-full text-[10px] font-bold bg-indigo-100 text-indigo-700 uppercase">
                                {{ \$session->type }}
                            </span>
                            <span class="font-bold text-sm text-slate-900 dark:text-slate-100">{{ \$session->username }}</span>
                        </div>
                        <span class="shrink-0 px-2.5 py-1 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-700 flex items-center gap-1">
                    ...
                    <div class="grid grid-cols-2 gap-y-3 gap-x-2 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 mb-4">
                        ...
                        <div>
                            <p class="text-[10px] text-slate-500">MAC Address</p>
                        ...
                        <div>
                            <p class="text-[10px] text-slate-500">Router</p>
        ...
        @empty
            <div class="text-center py-12 bg-white dark:bg-slate-800 rounded-2xl border border-slate-100 dark:border-slate-700/60 shadow-sm">
                <div class="w-16 h-16 rounded-full bg-slate-50 dark:bg-slate-700 flex items-center justify-center mx-auto mb-3">
                    <span class="material-symbols-outlined text-3xl text-slate-300 dark:text-slate-500">wifi_off</span>
                </div>
                <p class="text-slate-900 dark:text-slate-100 font-semibold mb-1">Tidak ada perangkat aktif</p>
                <p class="text-slate-500 text-xs px-6">Saat ini tidak ada koneksi online yang menggunakan layanan Anda.</p>
            </div>
        @endforelse
    </div>
</div>
...
echo "Updated active-sessions view.\n";
?>
