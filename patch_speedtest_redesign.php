...
<div class="p-4 sm:p-6 min-h-[calc(100vh-4rem)] flex flex-col items-center pb-24" 
     x-data="{
        ...
        async startTest() {
            ...
            this.downloadSpeed = '-';
            this.ping = 'Mengukur...';
            this.jitter = 'Mengukur...';
            this.progress = 5;

            const pingTarget = '/build/manifest.json';

            try {
                const dynamicTargets = await this.loadManifestTargets();
                ...
                // Measure Ping & Jitter
                const pingResult = await this.measurePing(pingTarget, 5);
                this.ping = pingResult.avg > 0 ? (pingResult.avg + ' ms') : '-';
                this.jitter = pingResult.jitter > 0 ? (pingResult.jitter + ' ms') : '-';
                this.progress = 20;
                ...
                let lastProgress = 0;
                const rounds = 15; // More rounds for smoother animation
                ...
                for (let i = 0; i < rounds; i++) {
                    ...
                    if (elapsedMs > 0) {
                        ...
                        if (parseFloat(speedMbps) > lastProgress) {
                            this.currentSpeed = speedMbps;
                            lastProgress = parseFloat(speedMbps);
                        }
                    }
                    this.progress = 20 + ((i + 1) / rounds * 80);
                ...
                this.currentSpeed = finalMbps;
                this.downloadSpeed = finalMbps + ' Mbps';
                this.progress = 100;

            } catch (e) {
                console.error('Speedtest error:', e);
                this.currentSpeed = 'Error';
                this.downloadSpeed = 'Gagal';
                this.ping = 'Gagal';
                this.jitter = 'Gagal';
                this.progress = 0;
            } finally {
                setTimeout(() => { this.isTesting = false; }, 500);
            }
        }
     }">
    ...
    <div class="w-full max-w-sm mt-auto">
        <button type="button" @click="startTest()" x-bind:disabled="isTesting"
                ...
                :class="isTesting ? 'bg-slate-100 text-slate-400 dark:bg-slate-800 dark:text-slate-500 cursor-not-allowed' : 'bg-slate-900 text-white dark:bg-white dark:text-slate-900 hover:shadow-xl hover:shadow-slate-900/20'">
