<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="<?php echo e(route('billing.invoices.show', $invoice->id)); ?>" class="p-2 text-slate-500 hover:text-slate-700 rounded-lg hover:bg-slate-100">
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500"><?php echo e($invoice->invoice_number); ?></p>
        </div>
    </div>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <div class="grid grid-cols-12 gap-4 items-end p-4 bg-slate-50 rounded-lg">
                            <div class="col-span-6">
                                ...
                                <input type="text" wire:model="items.<?php echo e($index); ?>.description" class="w-full px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Qty</label>
                                <input type="number" wire:model="items.<?php echo e($index); ?>.quantity" wire:change="calculateTotal" min="1" class="w-full px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Harga Satuan</label>
                                <input type="number" wire:model="items.<?php echo e($index); ?>.unit_price" wire:change="calculateTotal" min="0" class="w-full px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                            </div>
                            <div class="col-span-1">
                                <label class="block text-sm font-medium text-slate-700 mb-1">Total</label>
                                <input type="text" wire:model="items.<?php echo e($index); ?>.total" readonly class="w-full px-4 py-2 bg-slate-100 border border-slate-200 rounded-lg text-sm text-slate-600">
                            </div>
                            <div class="col-span-1">
                                <button type="button" wire:click="removeItem(<?php echo e($index); ?>)" class="p-2 text-red-500 hover:bg-red-50 rounded-lg">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
unset($__errorArgs, $__bag); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            ...
            <div class="flex items-center justify-end gap-4 pt-6 border-t border-slate-200">
                <div class="text-right">
                    ...
                    <p class="text-2xl font-bold text-slate-900">Rp <?php echo e(number_format($total_amount, 0, ',', '.')); ?></p>
                </div>
            </div>

            <div class="flex items-center justify-end gap-4 pt-4">
                <a href="<?php echo e(route('billing.invoices.show', $invoice->id)); ?>" class="px-6 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                    Batal
                </a>
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\billing\invoice\edit.blade.php ENDPATH**/ ?>