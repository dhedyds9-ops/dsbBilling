<div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_0 = true; $__currentLoopData = $invoices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-300 font-medium">
                                <?php echo e($row->invoice_number); ?>

                                <div class="text-xs text-slate-400 font-normal mt-0.5"><?php echo e($row->created_at->format('d M Y')); ?></div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 font-bold text-slate-700 dark:text-slate-300">
                                Rp <?php echo e(number_format($row->total_amount ?? 0, 0, ',', '.')); ?>

                            </td>
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">
                                <?php echo e($row->due_date ? $row->due_date->format('d M Y') : '-'); ?>

                            </td>
                            <td class="px-4 py-3">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($row->status === 'paid'): ?>
                            ...
                            <td class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="<?php echo e(route('reseller-portal.billing.invoices.show', $row->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-blue-50 hover:bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 dark:text-blue-400 transition-colors" title="Detail">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:18px">visibility</span>
                                    </a>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($row->status, ['unpaid', 'partial', 'overdue', 'pending'])): ?>
                                        <button wire:click="openPaymentModal(<?php echo e($row->id); ?>)" type="button" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-emerald-50 hover:bg-emerald-100 text-emerald-600 dark:bg-emerald-900/20 dark:hover:bg-emerald-900/40 dark:text-emerald-400 transition-colors" title="Proses Bayar"> <span class="material-symbols-outlined notranslate" translate="no" style="font-size:18px">payments</span> </button>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </div>
                            </td>
                        </tr>
    ...
    <div x-data="{ show: <?php if ((object) ('showDetailModal') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('showDetailModal'->value()); ?>')<?php echo e('showDetailModal'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('showDetailModal'); ?>')<?php endif; ?> }" x-show="show" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true" style="display: none;">
        ...
        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                <div x-show="show" x-transition.scale.origin.bottom sm.origin.center class="relative transform overflow-x-auto rounded-xl bg-white dark:bg-slate-800 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-2xl border border-slate-200 dark:border-slate-700">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedInvoice): ?>
                    <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex justify-between items-center">
                        <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100" id="modal-title">Detail Tagihan: <?php echo e($selectedInvoice->invoice_number); ?></h3>
                    ...
                    <div class="px-6 py-6">
                        <div class="flex justify-between mb-6">
                            <div>
                                ...
                                <p class="font-bold text-slate-800 dark:text-slate-200"><?php echo e($selectedInvoice->customer->name ?? '-'); ?></p>
                                <p class="text-sm text-slate-600 dark:text-slate-400"><?php echo e($selectedInvoice->customer->username ?? '-'); ?></p>
                            </div>
                            <div class="text-right">
                                <p class="text-xs text-slate-500 dark:text-slate-400 uppercase tracking-wider font-semibold mb-1">Status</p>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($selectedInvoice->status === 'paid'): ?>
                                ...
                                <?php elseif($selectedInvoice->status === 'unpaid'): ?>
                                ...
                                <?php else: ?>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300"><?php echo e(ucfirst($selectedInvoice->status)); ?></span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <p class="text-xs text-slate-500 dark:text-slate-400 mt-2">Jatuh Tempo: <?php echo e($selectedInvoice->due_date ? $selectedInvoice->due_date->format('d M Y') : '-'); ?></p>
                            </div>
                        </div>
                        ...
                        <table class="w-full text-sm mb-4">
                            ...
                            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_0 = true; $__currentLoopData = $selectedInvoice->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                                    <tr>
                                        <td class="py-3 text-slate-700 dark:text-slate-300"><?php echo e($item->description); ?></td>
                                        <td class="py-3 text-center text-slate-700 dark:text-slate-300"><?php echo e($item->quantity); ?></td>
                                        <td class="py-3 text-right font-mono text-slate-700 dark:text-slate-300">Rp <?php echo e(number_format($item->subtotal, 0, ',', '.')); ?></td>
                                    </tr>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_0): ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                                    <tr>
                            ...
                            <tfoot>
                                <tr class="border-t-2 border-slate-200 dark:border-slate-700">
                                    ...
                                    <th class="py-3 text-right font-bold font-mono text-indigo-600 dark:text-indigo-400 text-lg">Rp <?php echo e(number_format($selectedInvoice->total_amount, 0, ',', '.')); ?></th>
                                </tr>
                            </tfoot>
                        </table>
                    </div>
                    <div class="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700 flex justify-end gap-3">
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($selectedInvoice->status, ['unpaid', 'partial', 'overdue', 'pending'])): ?>
                            <button type="button" wire:click="openPaymentModal(<?php echo e($selectedInvoice->id); ?>)" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-1.5">
                ...
                <div class="relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                    ...
                    <div class="p-5 space-y-4">
                        <div class="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 p-3 rounded-lg text-sm border border-blue-100 dark:border-blue-800">
                            Sisa tagihan yang harus dibayar: <strong>Rp <?php echo e(number_format($paymentInvoiceTotal, 0, ',', '.')); ?></strong>
                        </div>
...
</div><?php /**PATH D:\dsBilling\resources\views\livewire\reseller-portal\billing\invoices.blade.php ENDPATH**/ ?>