<div class="space-y-6 pb-10">
    ...
    <div class="flex items-center justify-between flex-wrap gap-4">
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('isp.olts.index')); ?>" class="flex items-center text-slate-500 hover:text-slate-800 dark:text-slate-200 dark:hover:text-slate-100 transition-colors">
            ...
            <div class="w-10 h-10 rounded-full bg-indigo-100 dark:bg-indigo-900/50 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-base">
                <?php echo e(substr($olt->name ?? 'U', 0, 1)); ?>

            </div>
            <div>
                <h1 class="text-2xl font-black text-slate-800 dark:text-slate-100"><?php echo e($olt->name); ?></h1>
                <div class="flex items-center gap-3 mt-1 text-sm text-slate-500 dark:text-slate-400">
                    <span class="font-mono text-xs font-semibold text-indigo-600 dark:text-indigo-400"><?php echo e($olt->code); ?></span>
                    <span class="px-2.5 py-0.5 font-semibold rounded-md text-xs
                        <?php echo e($olt->status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-slate-100 text-slate-600 dark:bg-slate-700 dark:text-slate-400'); ?>">
                        <?php echo e($olt->status === 'active' ? 'Online' : 'Offline'); ?>

                    </span>
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($olt->uptime_text): ?>
                        <span class="font-mono text-xs bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded text-slate-600 dark:text-slate-400">Uptime: <?php echo e($olt->uptime_text); ?></span>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
            </div>
        </div>
        <div class="flex items-center gap-2">
            ...
            <a href="<?php echo e(route('isp.olts.edit', $olt->id)); ?>"
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-3 gap-3 pb-4 border-b border-slate-200 dark:border-slate-700">
                        <div class="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700">
                            ...
                            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($olt->ponPorts->count()); ?></p>
                        </div>
                        <div class="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-lg border border-emerald-100 dark:border-emerald-800">
                            ...
                            <p class="text-2xl font-bold text-emerald-700 dark:text-emerald-300"><?php echo e($olt->onus->where('last_seen_at', '>=', now()->subMinutes(5))->count()); ?></p>
                        </div>
                        <div class="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-100 dark:border-slate-700">
                            <p class="text-xs text-slate-500 dark:text-slate-400 mb-1">ONU Offline</p>
                            <p class="text-2xl font-bold text-slate-600 dark:text-slate-400"><?php echo e($olt->onus->filter(fn($o) => !$o->last_seen_at || $o->last_seen_at < now()->subMinutes(5))->count()); ?></p>
                        </div>
                    </div>

                    <div class="max-h-72 overflow-y-auto space-y-2 pr-1">
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_0 = true; $__currentLoopData = $olt->ponPorts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $port): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <?php
                            $portOnline  = $port->onus->where('status', 'active')->count();
                            $portOffline = $port->onus->where('status', 'inactive')->count();
                            $portTotal   = $port->onus->count();
                        ?>
                        <a href="<?php echo e(route('isp.onus.index', ['ponFilter' => $port->id, 'oltFilter' => $olt->id])); ?>"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-md bg-indigo-50 dark:bg-indigo-900/30 group-hover:bg-indigo-600 group-hover:text-white dark:group-hover:bg-indigo-500 text-indigo-600 dark:text-indigo-400 flex items-center justify-center font-bold text-sm transition-colors">
                                    <?php echo e($port->port_number ?? $loop->iteration); ?>

                                </div>
                                <div>
                                    <p class="text-sm font-semibold text-slate-800 dark:text-slate-200"><?php echo e($port->name ?? 'PON ' . $port->port_number); ?></p>
                                    <p class="text-xs text-slate-500 dark:text-slate-400"><?php echo e($port->type ? strtoupper($port->type) . ' Port' : 'Gpon Port'); ?></p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($portOnline > 0): ?>
                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">
                                    <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 inline-block"></span><?php echo e($portOnline); ?>

                                </span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($portOffline > 0): ?>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\isp\olt\show.blade.php ENDPATH**/ ?>