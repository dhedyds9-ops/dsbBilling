<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($survey->customer_name); ?></h1>
            <div class="flex items-center gap-2 mt-1">
                <span class="px-3 py-1 text-xs font-medium rounded-full 
                    <?php if($survey->status === 'scheduled'): ?> bg-blue-100 dark:bg-blue-900/50 text-blue-600
                    <?php elseif($survey->status === 'in_progress'): ?> bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600
                    <?php elseif($survey->status === 'completed'): ?> bg-green-100 dark:bg-green-900/50 text-green-600
                ...
                <span class="text-sm text-slate-500 dark:text-slate-400"><?php echo e($survey->survey_date?->format('d/m/Y')); ?></span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('crm.surveys.edit', $survey->id)); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($survey->customer_name); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Alamat</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($survey->address ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tanggal Survey</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($survey->survey_date?->format('d/m/Y') ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Catatan</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($survey->notes ?? '-'); ?></p>
                    </div>
                </div>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\crm\survey\show.blade.php ENDPATH**/ ?>