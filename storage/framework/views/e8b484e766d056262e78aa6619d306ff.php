<!DOCTYPE html>
<html class="dark" lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>" style="scroll-behavior: smooth;">
<head>
    <meta charset="utf-8"/>
    <meta content="width=device-width, initial-scale=1.0" name="viewport"/>
    <title><?php echo e(config('app.name', 'dsBilling')); ?> - Reliable Fiber Optic Connection</title>
...
<header class="bg-surface/60 dark:bg-surface/60 backdrop-blur-3xl border-b border-white/10 shadow-[0_8px_32px_0_rgba(0,0,0,0.36)] fixed top-0 w-full z-50">
    <div class="flex justify-between items-center px-margin-mobile md:px-margin-desktop py-4 max-w-container-max mx-auto">
        <!-- Brand -->
        <a href="<?php echo e(route('home')); ?>" class="font-display-lg text-headline-lg font-black text-primary tracking-tighter">
        ...
        <div class="hidden md:flex items-center gap-stack-md">
            <a href="<?php echo e(route('guest.payment')); ?>" class="inline-flex btn-secondary text-primary font-label-bold text-label-bold px-6 py-2 rounded-full">
                Cek Tagihan
            </a>
            <a href="<?php echo e(route('customer.login')); ?>" class="btn-primary text-white font-label-bold text-label-bold px-6 py-2 rounded-full flex items-center gap-2">
    ...
    <div x-show="mobileMenuOpen" x-collapse x-cloak class="md:hidden bg-surface-container-high border-t border-white/10 px-margin-mobile py-4 shadow-xl">
        <div class="flex flex-col gap-4">
            ...
            <a href="<?php echo e(route('guest.payment')); ?>" class="text-center btn-secondary text-primary font-label-bold text-label-bold px-6 py-3 rounded-full">
                Cek Tagihan
            </a>
            <a href="<?php echo e(route('customer.login')); ?>" class="text-center btn-primary text-white font-label-bold text-label-bold px-6 py-3 rounded-full flex items-center justify-center gap-2">
                <span class="material-symbols-outlined text-sm">account_balance_wallet</span>
                Portal Pelanggan
            </a>
        </div>
...
<main class="pt-24 md:pt-32 pb-24">
    ...
    <section class="relative min-h-[80vh] flex items-center justify-center px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto mb-32">
        <div class="absolute inset-0 z-[-1] overflow-hidden rounded-3xl">
            ...
            <div class="w-full h-full bg-cover bg-center opacity-30" style="background-image: url('<?php echo e(asset('images/fiber_network.jpg')); ?>')"></div>
                </div>
        ...
        <div class="text-center z-10 max-w-4xl">
            ...
            <p class="font-body-lg text-body-lg text-on-surface-variant mb-stack-lg max-w-2xl mx-auto">
                Rasakan kecepatan internet tanpa batas dengan teknologi Full Fiber Optic dari <?php echo e($companyName); ?>. Stabil, cepat, dan dapat diandalkan untuk segala kebutuhan digital Anda.
            </p>
            <div class="flex flex-col sm:flex-row justify-center gap-stack-md">
                ...
                <a class="btn-secondary text-primary font-label-bold text-label-bold px-8 py-4 rounded-full text-center" href="https://wa.me/<?php echo e(preg_replace('/\D/', '', $companyPhone)); ?>" target="_blank">
                    Hubungi Kami
                </a>
            </div>
        </div>
    ...
    <!-- Internet Packages -->
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showPppoe->isNotEmpty()): ?>
    <section class="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto mb-32" id="paket">
        ...
        <div class="flex  lg:grid lg:grid-cols-3 gap-gutter items-center pb-8 snap-x snap-mandatory hide-scrollbar -mx-margin-mobile px-margin-mobile lg:mx-0 lg:px-0">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $showPppoe; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $pkg): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($index == 1 || (count($showPppoe) == 1)): ?>
                    <!-- Package 2 (Popular) -->
                    <div class="w-[85vw] sm:w-[350px] lg:w-auto shrink-0 snap-center glass-card p-10 rounded-[2rem] flex flex-col border-[1.5px] border-primary/50 shadow-[0_0_30px_rgba(6,182,212,0.15)] relative transform lg:-translate-y-4 z-10 bg-surface-container-high/80 h-full">
                        ...
                        <div class="mb-8 mt-2">
                            <span class="inline-block px-3 py-1 bg-primary/20 text-primary font-label-sm text-label-sm rounded-full mb-4"><?php echo e($pkg->name); ?></span>
                            <h3 class="font-display-lg text-[48px] text-on-surface leading-none mb-2 text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary"><?php echo e(preg_replace('/[^0-9]/', '', $pkg->description ?? $pkg->name ?? '30')); ?> <span class="text-xl font-normal text-on-surface-variant">Mbps</span></h3>
                            <p class="font-headline-md text-headline-md text-primary">Rp <?php echo e(number_format($pkg->price ?? $pkg->base_price ?? 0, 0, ',', '.')); ?><span class="text-sm font-normal text-on-surface-variant">/bln</span></p>
                        </div>
                        <ul class="space-y-4 mb-8 flex-grow">
                            <li class="flex items-center gap-3 text-on-surface font-body-md text-body-md">
                                ...
                                <?php echo e($pkg->feature1 ?? 'Sangat Cepat & Stabil'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($pkg->feature2 ?? 'Cocok Untuk Keluarga'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($pkg->feature3 ?? 'Unlimited Quota'); ?>

                            </li>
                        </ul>
                        <button @click.prevent="pkgName = '<?php echo e(addslashes($pkg->name)); ?>'; regOpen = true" class="w-full btn-primary text-center text-white font-label-bold text-label-bold py-4 rounded-xl mt-auto transition-all">
                ...
                <?php else: ?>
                    ...
                    <div class="w-[85vw] sm:w-[350px] lg:w-auto shrink-0 snap-center glass-card p-8 rounded-[2rem] flex flex-col glow-hover transition-all duration-300 lg:translate-y-4 h-full">
                        <div class="mb-8">
                            <span class="inline-block px-3 py-1 bg-surface-variant text-on-surface-variant font-label-sm text-label-sm rounded-full mb-4"><?php echo e($pkg->name); ?></span>
                            <h3 class="font-display-lg text-[40px] text-on-surface leading-none mb-2"><?php echo e(preg_replace('/[^0-9]/', '', $pkg->description ?? $pkg->name ?? '10')); ?> <span class="text-xl font-normal text-on-surface-variant">Mbps</span></h3>
                            <p class="font-headline-md text-headline-md text-primary">Rp <?php echo e(number_format($pkg->price ?? $pkg->base_price ?? 0, 0, ',', '.')); ?><span class="text-sm font-normal text-on-surface-variant">/bln</span></p>
                        </div>
                        <ul class="space-y-4 mb-8 flex-grow">
                            <li class="flex items-center gap-3 text-on-surface-variant font-body-md text-body-md">
                                ...
                                <?php echo e($pkg->feature1 ?? 'Koneksi Stabil'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface-variant font-body-md text-body-md">
                                <span class="material-symbols-outlined text-tertiary-fixed-dim text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($pkg->feature2 ?? 'Browsing Lancar'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface-variant font-body-md text-body-md">
                                <span class="material-symbols-outlined text-tertiary-fixed-dim text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($pkg->feature3 ?? 'Tanpa FUP'); ?>

                            </li>
                        </ul>
                        <button @click.prevent="pkgName = '<?php echo e(addslashes($pkg->name)); ?>'; regOpen = true" class="w-full btn-secondary text-center text-primary font-label-bold text-label-bold py-3 rounded-xl mt-auto transition-all">
                            Pilih Paket
                        </button>
                    </div>
    ...
    <!-- Hotspot Section -->
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showHotspotVoucher->isNotEmpty()): ?>
    <section class="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto mb-32 bg-surface-container/30 rounded-3xl p-8 md:p-16 border border-white/5" id="hotspot">
        ...
        <div class="flex  lg:grid lg:grid-cols-4 gap-4 sm:gap-6 pb-8 snap-x snap-mandatory hide-scrollbar -mx-margin-mobile px-margin-mobile lg:mx-0 lg:px-0">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $showHotspotVoucher; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $hot): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
            <div class="w-[75vw] sm:w-[280px] lg:w-auto shrink-0 snap-center relative bg-surface-container-high/80 rounded-xl overflow-hidden flex flex-col border border-white/10 shadow-lg h-full" style="mask-composite: source-in;">
                <div class="p-6 flex-grow bg-gradient-to-br from-primary/10 to-secondary/5">
                    <div class="flex justify-between items-start mb-4">
                        <div class="flex flex-col">
                            <span class="text-on-surface font-headline-md text-headline-md"><?php echo e($hot->name); ?></span>
                            <span class="text-primary/70 text-label-sm"><?php echo e($hot->description ?? 'Akses Unlimited'); ?></span>
                        </div>
                        <span class="material-symbols-outlined text-primary/70 text-3xl"><?php echo e($hot->icon ?? 'timer'); ?></span>
                    </div>
                ...
                <div class="p-6 bg-surface-container-highest/50">
                    <div class="flex justify-between items-center mb-4">
                        <span class="text-primary font-display-lg text-headline-md">Rp <?php echo e(number_format($hot->price ?? $hot->base_price ?? 0, 0, ',', '.')); ?></span>
                        <span class="material-symbols-outlined text-on-surface-variant">confirmation_number</span>
                    </div>
                    <button @click.prevent="vouchId = '<?php echo e($hot->id); ?>'; vouchName = '<?php echo e(addslashes($hot->name)); ?>'; vouchPrice = 'Rp <?php echo e(number_format($hot->price ?? $hot->base_price ?? 0, 0, ',', '.')); ?>'; vouchOpen = true" class="w-full btn-primary text-white py-3 rounded-lg text-sm font-label-bold transition-all mt-auto">Beli Voucher</button>
                </div>
            </div>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
        </div>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showMemberHotspot->isNotEmpty()): ?>
    <section class="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto mb-32" id="member-hotspot">
        ...
        <div class="flex  lg:grid lg:grid-cols-3 gap-gutter pb-8 snap-x snap-mandatory hide-scrollbar -mx-margin-mobile px-margin-mobile lg:mx-0 lg:px-0">
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $showMemberHotspot; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $mem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($index == 1 || (count($showMemberHotspot) == 1)): ?>
                    <div class="w-[85vw] sm:w-[350px] lg:w-auto shrink-0 snap-center glass-card p-8 rounded-[2rem] flex flex-col glow-hover transition-all duration-300 border-[1.5px] border-primary/30 bg-surface-container-high/80 shadow-[0_0_30px_rgba(6,182,212,0.1)] h-full">
                        <div class="mb-8">
                            <span class="inline-block px-3 py-1 bg-primary/20 text-primary font-label-sm text-label-sm rounded-full mb-4"><?php echo e($mem->description ?? 'Pro Member'); ?></span>
                            <h3 class="font-headline-md text-headline-md text-on-surface mb-2"><?php echo e($mem->name); ?></h3>
                            <p class="font-headline-md text-headline-md text-primary">Rp <?php echo e(number_format($mem->price ?? $mem->base_price ?? 0, 0, ',', '.')); ?><span class="text-sm font-normal text-on-surface-variant">/bln</span></p>
                    </div>
                        <ul class="space-y-4 mb-8 flex-grow">
                            <li class="flex items-center gap-3 text-on-surface font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($mem->feature1 ?? '2 Devices Concurrent'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($mem->feature2 ?? 'Priority Bandwidth'); ?>
                        ...
                        <button @click.prevent="pkgName = '<?php echo e(addslashes($mem->name)); ?>'; regOpen = true" class="w-full btn-primary text-white font-label-bold text-label-bold py-3 rounded-xl mt-auto">Daftar Member</button>
                    </div>
                <?php else: ?>
                    <div class="w-[85vw] sm:w-[350px] lg:w-auto shrink-0 snap-center glass-card p-8 rounded-[2rem] flex flex-col glow-hover transition-all duration-300 border border-white/10 h-full">
                        <div class="mb-8">
                            <span class="inline-block px-3 py-1 bg-surface-variant text-on-surface-variant font-label-sm text-label-sm rounded-full mb-4"><?php echo e($mem->description ?? 'Starter Member'); ?></span>
                            <h3 class="font-headline-md text-headline-md text-on-surface mb-2"><?php echo e($mem->name); ?></h3>
                            <p class="font-headline-md text-headline-md text-primary">Rp <?php echo e(number_format($mem->price ?? $mem->base_price ?? 0, 0, ',', '.')); ?><span class="text-sm font-normal text-on-surface-variant">/bln</span></p>
                </div>
                        <ul class="space-y-4 mb-8 flex-grow">
                            <li class="flex items-center gap-3 text-on-surface-variant font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($mem->feature1 ?? 'Single Device Login'); ?>

                            </li>
                            <li class="flex items-center gap-3 text-on-surface-variant font-body-md text-body-md">
                                <span class="material-symbols-outlined text-primary text-sm" style="font-variation-settings: 'FILL' 1;">check_circle</span>
                                <?php echo e($mem->feature2 ?? 'Standard Bandwidth'); ?>
                        ...
                        <button @click.prevent="pkgName = '<?php echo e(addslashes($mem->name)); ?>'; regOpen = true" class="w-full btn-secondary text-primary font-label-bold text-label-bold py-3 rounded-xl mt-auto">Daftar Member</button>
                    </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
        </div>
    ...
    <section class="px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto">
        <div class="relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-surface-container-high to-surface p-8 md:p-12 border border-primary/20 shadow-[0_0_40px_rgba(6,182,212,0.1)] flex flex-col md:flex-row items-center justify-between gap-8">
            ...
            <div class="z-10 flex-shrink-0">
                <a href="<?php echo e(route('guest.payment')); ?>" class="bg-[#1bbd85] hover:bg-[#4edea3] text-on-tertiary font-label-bold text-label-bold px-8 py-4 rounded-xl flex items-center gap-3 transition-colors shadow-[0_0_20px_rgba(27,189,133,0.3)] animate-pulse hover:animate-none">
...
<footer class="bg-surface-container-lowest dark:bg-surface-container-lowest border-t border-outline-variant w-full py-stack-lg px-margin-mobile md:px-margin-desktop grid grid-cols-1 md:grid-cols-4 gap-gutter items-center">
    <div class="flex flex-col gap-2 md:col-span-2">
        ...
        <p class="font-body-md text-body-md text-on-surface-variant flex items-center gap-4">
            <span>&copy; <?php echo e(date('Y')); ?> <?php echo e($companyName); ?>. Reliable Fiber Optic Connection.</span>
            <a href="<?php echo e(route('admin.login')); ?>" class="text-on-surface-variant hover:text-primary transition-colors flex items-center gap-1 opacity-50 hover:opacity-100" title="Admin Login">
...
<div x-show="regOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    ...
    <div x-show="regOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-[2rem] bg-surface-container-high border border-white/10 text-left shadow-2xl transition-all w-full max-w-md p-8">
        ...
        <form action="<?php echo e(route('register.lead')); ?>" method="POST" class="space-y-4">
            <?php echo csrf_field(); ?>
...
<div x-show="vouchOpen" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6" aria-labelledby="modal-voucher" role="dialog" aria-modal="true">
    ...
    <div x-show="vouchOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-[2rem] bg-surface-container-high border border-white/10 text-left shadow-2xl transition-all w-full max-w-md p-8">
        ...
        <form action="<?php echo e(route('buy-voucher-guest')); ?>" method="POST" class="space-y-4">
            <?php echo csrf_field(); ?>
...
<?php /**PATH D:\dsBilling\resources\views/landing.blade.php ENDPATH**/ ?>