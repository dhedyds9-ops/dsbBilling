<div class="space-y-6">
    <div class="flex items-center gap-4">
        <a href="<?php echo e(route('isp.pppoe-users.index')); ?>" class="p-2 text-slate-500 hover:text-slate-700 rounded-lg hover:bg-slate-100">
        ...
        <div class="flex-1">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900"><?php echo e($pppoeUser->username); ?></h1>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="px-3 py-1 text-xs font-medium rounded-full 
                            <?php if($pppoeUser->status === 'active'): ?> bg-green-100 text-green-600
                            <?php elseif($pppoeUser->status === 'inactive'): ?> bg-slate-100 text-slate-600
                            <?php elseif($pppoeUser->status === 'suspended'): ?> bg-yellow-100 text-yellow-600
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('isp.pppoe-users.edit', $pppoeUser->id)); ?>" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-slate-900 font-mono"><?php echo e($pppoeUser->username); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Service Profile</label>
                        <p class="text-slate-900"><?php echo e($pppoeUser->serviceProfile?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Status</label>
                        <p class="text-slate-900"><?php echo e(ucfirst($pppoeUser->status)); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Dibuat Oleh</label>
                        <p class="text-slate-900"><?php echo e($pppoeUser->createdBy?->name ?? '-'); ?></p>
                    </div>
                </div>
            </div>
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\isp\pppoe-user\show.blade.php ENDPATH**/ ?>