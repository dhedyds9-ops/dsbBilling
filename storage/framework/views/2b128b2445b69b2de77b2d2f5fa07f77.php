...
<div>
    ...
    <div x-data="{ subtitle: 'Ringkasan untuk tanggal: <?php echo e(\Carbon\Carbon::parse($filters['date'] ?? now())->translatedFormat('l, d F Y')); ?>' }" x-effect="if(document.getElementById('dynamic-subtitle-harian')) document.getElementById('dynamic-subtitle-harian').innerHTML = subtitle"></div>
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-slate-800 dark:text-slate-100">Pemasukan Harian</h1>
        <p class="text-slate-500 dark:text-slate-400 mt-1">Ringkasan Periode : <span class="text-emerald-600 font-bold dark:text-emerald-500"><?php echo e(\Carbon\Carbon::parse($filters['date'] ?? now())->translatedFormat('l, d F Y')); ?></span></p>
    </div>
                    ...
                    <div x-data="{ open: false }" @click.away="open = false" class="relative z-50">
                        ...
                        <div x-show="open" x-transition style="display: none;" class="absolute top-full left-0 mt-2 w-64 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl p-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <button @click="open = false; $wire.set('filters.date', '<?php echo e(now()->toDateString()); ?>')" type="button" class="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-700 rounded-lg text-sm font-medium transition-colors">
                                    Hari Ini
                                </button>
                            </div>
                        </div>
...
</div><?php /**PATH D:\dsBilling\resources\views/livewire/keuangan/income-harian/index.blade.php ENDPATH**/ ?>