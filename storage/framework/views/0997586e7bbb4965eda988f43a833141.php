        ...
        <div class="p-4 space-y-2">
          <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $chatIds; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $c): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
            <div class="grid grid-cols-12 gap-2 items-center p-2 rounded bg-slate-50 dark:bg-slate-700/40 border border-slate-100 dark:border-slate-700">
              <div class="col-span-4"><input type="text" wire:model="chatIds.<?php echo e($i); ?>.id" placeholder="-100xxxxxx (Group) / id pribadi" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono"></div>
              <div class="col-span-3"><input type="text" wire:model="chatIds.<?php echo e($i); ?>.name" placeholder="Label (Nama Group)" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100"></div>
              ...
              <div class="col-span-2"><input type="text" wire:model="chatIds.<?php echo e($i); ?>.events" placeholder="billing,ticket,alarm" class="w-full px-2.5 py-1.5 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100"></div>
              <div class="col-span-1 flex items-center gap-1 justify-end">
                <label class="inline-flex text-xs"><input type="checkbox" wire:model="chatIds.<?php echo e($i); ?>.enabled" class="rounded border-slate-300"></label>
                <button type="button" wire:click="removeChatId(<?php echo e($i); ?>)" class="p-1 text-slate-400 hover:text-red-600"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
              </div>
            </div>
            ...
            <div class="p-4">
              <div class="flex items-center justify-between mb-2 flex-wrap gap-2">
                <div class="flex items-center gap-2">
                  <label class="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300"><input type="checkbox" wire:model="templates.<?php echo e($i); ?>.enabled" class="rounded border-slate-300"> Aktif</label>
              ...
              <textarea wire:model="templates.<?php echo e($i); ?>.content" rows="4" class="w-full px-3 py-2 text-xs border border-slate-200 dark:border-slate-700 rounded-md bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 font-mono leading-relaxed"></textarea>
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\pengaturan\telegram\index.blade.php ENDPATH**/ ?>