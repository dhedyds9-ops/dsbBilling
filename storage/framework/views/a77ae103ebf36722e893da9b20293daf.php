<div>
    ...
    <div class="space-y-4">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div wire:click="$set('filters.status', '')" class="relative overflow-x-auto rounded-xl border border-emerald-200 dark:border-emerald-800/60 shadow-sm bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-950/50 dark:to-slate-800 group hover:shadow-md transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight">Rp <?php echo e(number_format($stats['total_amount'] ?? 0, 0, ',', '.')); ?></span>
                    </div>
                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-1"><?php echo e(number_format($stats['total'] ?? 0)); ?> Transaksi</p>
                </div>
            </div>
            ...
            <div wire:click="$set('filters.status', 'success')" class="relative overflow-x-auto rounded-xl border border-blue-200 dark:border-blue-800/60 shadow-sm bg-gradient-to-br from-blue-50 to-white dark:from-blue-950/50 dark:to-slate-800 group hover:shadow-md transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight"><?php echo e(number_format($stats['success'] ?? 0)); ?></span>
            ...
            <div wire:click="$set('filters.status', 'pending')" class="relative overflow-x-auto rounded-xl border border-amber-200 dark:border-amber-800/60 shadow-sm bg-gradient-to-br from-amber-50 to-white dark:from-amber-950/50 dark:to-slate-800 group hover:shadow-md transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight"><?php echo e(number_format($stats['pending'] ?? 0)); ?></span>
                        <span class="text-sm font-medium text-slate-500 dark:text-slate-400">Transaksi</span>
                    </div>
                </div>
            </div>
            ...
            <div wire:click="$set('filters.status', 'failed')" class="relative overflow-x-auto rounded-xl border border-red-200 dark:border-red-800/60 shadow-sm bg-gradient-to-br from-red-50 to-white dark:from-red-950/50 dark:to-slate-800 group hover:shadow-md transition-all cursor-pointer">
                ...
                <div class="p-4 pt-5">
                    ...
                    <div class="flex items-baseline gap-2">
                        <span class="text-2xl font-black text-slate-800 dark:text-slate-100 tracking-tight"><?php echo e(number_format($stats['failed'] ?? 0)); ?></span>
                        <span class="text-sm font-medium text-slate-500 dark:text-slate-400">Transaksi</span>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 items-center justify-between bg-white dark:bg-slate-800 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="<?php echo e(route('billing.payments.create')); ?>" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
            ...
            <div class="absolute top-0 left-0 w-full h-12 bg-indigo-50 dark:bg-indigo-900/30 border-b border-indigo-100 dark:border-indigo-800 flex items-center justify-between px-4 z-10">
                <div class="flex items-center gap-3">
                    <span class="text-sm font-semibold text-indigo-700 dark:text-indigo-300">
                        <span class="w-5 h-5 inline-flex items-center justify-center bg-indigo-600 text-white rounded-full text-xs mr-1"><?php echo e(count($selected)); ?></span> Terpilih
                    </span>
                </div>
                <div class="flex items-center gap-2">
                    <button wire:click="bulkDelete" wire:confirm="Yakin ingin menghapus <?php echo e(count($selected)); ?> pembayaran terpilih secara permanen?" class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-medium transition-colors shadow-sm flex items-center gap-1.5">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $payments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3 text-center">
                                <input type="checkbox" wire:model.live="selected" value="<?php echo e($payment->id); ?>" class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 bg-slate-50 dark:bg-slate-900/50 dark:border-slate-600 cursor-pointer">
                            </td>
                            ...
                            <td class="px-4 py-3 text-slate-700 dark:text-slate-300">
                                <div class="font-medium"><?php echo e($payment->customer->name ?? '-'); ?></div>
                            </td>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($payment->invoices->count() > 0): ?>
        <a href="<?php echo e(route('billing.invoices.show', $payment->invoices->first()->id)); ?>" class="text-blue-600 dark:text-blue-400 hover:underline">
            <?php echo e($payment->invoices->first()->invoice_number); ?>
    ...
    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400">
                                <?php echo e($payment->paid_at?->format('d M Y, H:i') ?? '-'); ?>

                            </td>
                            <td class="px-4 py-3 text-slate-600 dark:text-slate-400 uppercase text-xs font-semibold">
                                <?php echo e(str_replace('_', ' ', $payment->method)); ?>

                            </td>
                            <td class="px-4 py-3">
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($payment->status === 'success'): ?>
                                ...
                                <?php else: ?>
                                    <span class="inline-flex items-center gap-1.5 px-2 py-1 rounded-md text-xs font-medium bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400"><?php echo e(ucfirst($payment->status)); ?></span>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($payment->invoices->count() > 0): ?>
        <a href="<?php echo e(route('billing.invoices.show', $payment->invoices->first()->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-blue-50 hover:bg-blue-100 text-blue-600 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 dark:text-blue-400 transition-colors" title="Print Invoice">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->hasRole('administrator')): ?>
        <button wire:click="delete(<?php echo e($payment->id); ?>)" onclick="confirm('Yakin ingin menghapus data pembayaran ini?') || event.stopImmediatePropagation()" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-red-50 hover:bg-red-100 text-red-600 dark:bg-red-900/20 dark:hover:bg-red-900/40 dark:text-red-400 transition-colors" title="Hapus">
...
</div><?php /**PATH D:\dsBilling\resources\views\livewire\billing\payment\index.blade.php ENDPATH**/ ?>