<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="<?php echo e(route('crm.activations.index')); ?>" class="p-2 text-slate-500 hover:text-slate-700 rounded-lg hover:bg-slate-100">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900"><?php echo e($activation->customer_name); ?></h1>
            <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-sm text-slate-500"><?php echo e($activation->activation_date?->format('d/m/Y')); ?></span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('crm.activations.edit', $activation->id)); ?>" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                Edit
            </a>
        </div>
    </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
                 ...
                 <?php $__env->slot('header', null, []); ?> 
                    <h3 class="text-lg font-semibold text-slate-900">Informasi Activation</h3>
                 <?php $__env->endSlot(); ?>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Nama Pelanggan</label>
                        <p class="text-slate-900"><?php echo e($activation->customer_name); ?></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-500 mb-1">Tanggal Aktivasi</label>
                            <p class="text-slate-900"><?php echo e($activation->activation_date?->format('d/m/Y') ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 mb-1">Paket Layanan</label>
                            <p class="text-slate-900"><?php echo e($activation->service_package ?? '-'); ?></p>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Catatan</label>
                        <p class="text-slate-900"><?php echo e($activation->notes ?? '-'); ?></p>
                    </div>
                </div>
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\crm\activation\show.blade.php ENDPATH**/ ?>