...
<?php endif; ?>
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?php echo e(route('aaa.vouchers.create')); ?>" class="px-4 py-2 bg-primary-600 hover:bg-primary-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 mt-1"><?php echo e($stats['available'] ?? 0); ?></p>
                </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600 mt-1"><?php echo e($stats['used'] ?? 0); ?></p>
                </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600 mt-1"><?php echo e($stats['expired'] ?? 0); ?></p>
                </div>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $vouchers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $voucher): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50">
                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-900 dark:text-slate-100 font-mono"><?php echo e($voucher->code); ?></div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($voucher->type ?? '-'); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-900 dark:text-slate-100"><?php echo e($voucher->value ? 'Rp ' . number_format($voucher->value, 0, ',', '.') : '-'); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($voucher->duration ? $voucher->duration . ' ' . $voucher->duration_unit : '-'); ?></td>
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($voucher->quota ? $voucher->quota . ' MB' : '-'); ?></td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 text-xs font-medium rounded-full 
                                    <?php if($voucher->status === 'available'): ?> bg-green-100 dark:bg-green-900/50 text-green-600
                                    <?php elseif($voucher->status === 'used'): ?> bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600
                                    <?php elseif($voucher->status === 'expired'): ?> bg-red-100 dark:bg-red-900/50 text-red-600
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400"><?php echo e($voucher->created_at?->format('d/m/Y') ?? '-'); ?></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route('aaa.vouchers.show', $voucher->id)); ?>" class="p-2 text-slate-500 dark:text-slate-400 hover:text-primary-600 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </a>
                                    <a href="<?php echo e(route('aaa.vouchers.edit', $voucher->id)); ?>" class="p-2 text-slate-500 dark:text-slate-400 hover:text-blue-600 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                        </svg>
                                    </a>
                                    <button wire:click="delete(<?php echo e($voucher->id); ?>)" wire:confirm="Yakin ingin menghapus voucher ini?" class="p-2 text-slate-500 dark:text-slate-400 hover:text-red-600 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
