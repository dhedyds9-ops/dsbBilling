<div>
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Estimasi keuntungan bersih Anda dari selisih harga jual dan harga modal pada periode <?php echo e($currentMonthName); ?>.</p>
        </div>
    </div>

    <div class="mb-4 flex justify-end">
    <a href="<?php echo e(route('reseller-portal.reports.commission.print')); ?>" target="_blank" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-700 flex items-center gap-2 transition-colors shadow-sm text-sm font-medium">
...
<!-- Summary Cards -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
        <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">Rp <?php echo e(number_format($grossRevenue, 0, ',', '.')); ?></p>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100">Rp <?php echo e(number_format($totalCost, 0, ',', '.')); ?></p>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm bg-gradient-to-br from-emerald-50 to-white dark:from-emerald-950/20 dark:to-slate-800">
            ...
            <p class="text-2xl font-bold text-emerald-600 dark:text-emerald-400">Rp <?php echo e(number_format($netMargin, 0, ',', '.')); ?></p>
        </div>
        ...
        <div class="bg-white dark:bg-slate-800 rounded-xl p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
            ...
            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e(number_format($profitMarginPercent, 1)); ?>%</p>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm overflow-hidden mt-6 mb-6">
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_0 = true; $__currentLoopData = $details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        ...
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50">
                            <td class="px-6 py-3 text-slate-600 dark:text-slate-400">
                                <?php echo e($row->updated_at->format('d M Y')); ?><br>
                                <span class="text-xs text-slate-400"><?php echo e($row->updated_at->format('H:i')); ?></span>
                            </td>
                            <td class="px-6 py-3">
                                <div class="font-medium text-slate-800 dark:text-slate-200"><?php echo e($row->customer->name ?? '-'); ?></div>
                                <div class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Inv: <?php echo e($row->invoice_number); ?></div>
                            </td>
                            <td class="px-6 py-3">
                                <?php
                                    $sp = $row->customer ? $row->customer->customerServices()->with('serviceProfile')->first() : null;
                                ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($sp && $sp->serviceProfile): ?>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-slate-100 text-slate-800 dark:bg-slate-800 dark:text-slate-300">
                                        <?php echo e($sp->serviceProfile->name); ?>
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($details->hasPages()): ?>
            <div class="px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50">
                <?php echo e($details->links(data: ['scrollTo' => false])); ?>

            </div>
        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
    </div>
</div>
<?php /**PATH D:\dsBilling\resources\views\livewire\reseller-portal\reports\commission.blade.php ENDPATH**/ ?>