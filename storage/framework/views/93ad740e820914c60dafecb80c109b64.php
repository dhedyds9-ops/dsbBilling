...
<div
    ...
    <div x-show="show" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100"
        ...
        <div class="px-5 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3">
            ...
            <div>
                <h3 class="font-semibold text-slate-900 dark:text-slate-100"><?php echo e($this->confirmTitle ?? 'Konfirmasi'); ?></h3>
                <p class="text-sm text-slate-600 dark:text-slate-400 mt-0.5"><?php echo e($this->confirmMessage ?? 'Lanjutkan aksi?'); ?></p>
            </div>
        </div>
        <div class="px-5 py-4 flex items-center justify-end gap-2 bg-slate-50 dark:bg-slate-800/70 rounded-b-xl">
            ...
            <button wire:click="handleConfirm" @click="show = false" class="px-4 py-1.5 text-sm font-medium rounded-md text-white <?php echo e($this->confirmBtnClass ?? 'bg-red-600 hover:bg-red-700'); ?>">
                <?php echo e($this->confirmBtnText ?? 'Ya'); ?>

            </button>
        </div>
    </div>
</div>
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\partials\enterprise\confirm-modal.blade.php ENDPATH**/ ?>