<div class="space-y-6">
    ...
    <div class="p-4 bg-red-50 dark:bg-red-900/30 border border-red-100 rounded-xl flex items-start gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-red-600 mt-0.5"><?php echo e($error); ?></p>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                    ...
                    <tr class="hover:bg-slate-50 dark:bg-slate-800/50 transition-colors <?php echo e(($device['IsBlocked'] ?? false) ? 'bg-red-50 dark:bg-red-900/30/50' : ''); ?>">
                        <td class="p-3">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-full <?php echo e(($device['IsBlocked'] ?? false) ? 'bg-red-100 text-red-600' : 'bg-blue-50 dark:bg-blue-900/30 text-primary-600'); ?> flex items-center justify-center shrink-0">
                                <?php
                                ...
                                <div>
                                    <div class="font-medium text-slate-900 dark:text-slate-100"><?php echo e($device['HostName'] ?? 'Unknown Device'); ?></div>
                                    <div class="text-xs text-slate-500 dark:text-slate-400">Host</div>
                                </div>
                            </div>
                        </td>
                        <td class="p-3  text-slate-600 dark:text-slate-400 font-mono"><?php echo e($device['IPAddress'] ?? '-'); ?></td>
                        <td class="p-3  text-slate-600 dark:text-slate-400 font-mono"><?php echo e($device['MACAddress'] ?? '-'); ?></td>
                        <td class="p-3">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($device['IsBlocked'] ?? false): ?>
                            ...
                            <?php elseif($device['Active'] ?? false): ?>
                        ...
                        <td class="p-3  text-right">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($device['IsBlocked'] ?? false): ?>
                                <button wire:click="unblockMac('<?php echo e($device['MACAddress']); ?>')" 
                            ...
                            <?php else: ?>
                                <button wire:click="blockMac('<?php echo e($device['MACAddress']); ?>')" 
...
<?php /**PATH D:\dsBilling\resources\views\livewire\customer-portal\self-service\connected-devices.blade.php ENDPATH**/ ?>