...
<?php endif; ?>
    ...
    <div class="flex justify-between items-center mb-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">PPPoE Users</h1>
            <p class="text-gray-500 mt-1">Kelola semua pelanggan PPPoE</p>
        </div>
        <div x-data="{ open: false }" class="relative inline-block text-left">
            ...
            <div x-show="open" @click.away="open = false" x-transition class="origin-top-right absolute right-0 mt-2 w-64 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                <div class="py-1">
                    <a href="<?php echo e(route('isp.pppoe-users.create')); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                ...
                <div class="py-1">
                    <button
                        ...
                        class="flex items-center gap-2 px-4 py-2 text-sm w-full text-left <?php if(empty($selectedIds)): ?> text-gray-300 cursor-not-allowed <?php else: ?> text-red-600 hover:bg-red-50 <?php endif; ?>">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($selectedIds)): ?>
        <div class="mb-3 flex items-center gap-2">
            <span class="px-3 py-1 bg-primary-100 text-primary-800 rounded-full text-sm font-medium">
                <?php echo e(count($selectedIds)); ?> PPPoE User Dipilih
            </span>
            <button wire:click="selectedIds = []; selectAll = false" class="text-sm text-gray-500 hover:text-gray-700">
                Batal Pilih
            </button>
        </div>
    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <div class="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
            <div class="w-full lg:w-1/3">
                ...
                <div class="relative">
                    ...
                    <input wire:model.live.debounce.300ms="search" type="text" class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition-all" placeholder="Cari username, nama, atau nomor HP...">
                </div>
            </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">ID Pelanggan</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Nama</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Paket Langganan</th>
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Owner Data</th>
                        <th scope="col" class="px-6 py-3 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $pppoeUsers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $user): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = ''.e($user->id).''; ?>wire:key="<?php echo e($user->id); ?>" class="hover:bg-gray-50 transition-colors <?php if($user->trashed()): ?> bg-red-50 <?php endif; ?>">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:model.live="selectedIds" value="<?php echo e($user->id); ?>" class="w-4 h-4 text-primary-600 border-gray-300 rounded focus:ring-primary-500" <?php if($user->trashed()): ?> disabled <?php endif; ?>>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <?php else: ?>
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo e($user->status === 'active' ? 'bg-green-100 text-green-800' : ($user->status === 'suspended' ? 'bg-yellow-100 text-yellow-800' : ($user->status === 'terminated' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'))); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900"><?php echo e($user->customer?->code ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-gray-900"><?php echo e($user->customer?->name ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900"><?php echo e($user->serviceProfile?->name ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-600"><?php echo e($user->createdBy?->name ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div x-data="{ open: false }" class="relative inline-block text-left">
                                    ...
                                    <div x-show="open" @click.away="open = false" x-transition class="origin-top-right absolute right-0 mt-2 w-56 rounded-lg shadow-lg bg-white ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 z-50">
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!$user->trashed()): ?>
                                            <div class="py-1">
                                                <a href="<?php echo e(route('isp.pppoe-users.show', $user->id)); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <a href="<?php echo e(route('isp.pppoe-users.edit', $user->id)); ?>" class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                ...
                                                <button
                                                    type="button"
                                                    wire:click="renew(<?php echo e($user->id); ?>)"
                                                    wire:loading.attr="disabled"
                                                    class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 w-full text-left">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h5M20 20v-5h-5M20 8a8 8 0 00-14.31-4.9M4 16a8 8 0 0014.31 4.9"></path>
                                                    </svg>
                                                    Renew
                                                    <span wire:loading class="ml-2 text-gray-400 text-xs">...</span>
                                                </button>
                                                <button
                                                    type="button"
                                                    wire:click="print(<?php echo e($user->id); ?>)"
                                                    wire:loading.attr="disabled"
                                                    class="flex items-center gap-2 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 w-full text-left">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2M17 9V7a2 2 0 00-2-2H9a2 2 0 00-2 2v2"></path>
                                                    </svg>
                                                    Print
                                                    <span wire:loading class="ml-2 text-gray-400 text-xs">...</span>
                                                </button>
                                            ...
                                            <div class="py-1">
                                                <button
                                                    ...
                                                    <?php echo e($user->status === 'active' ? 'Suspend' : ($user->status === 'suspended' ? 'Aktifkan' : 'Ubah Status')); ?>
                                            ...
                                            <div class="py-1">
                                                <button
                                                    ...
                                                    wire:loading.attr="disabled"
                                                    wire:confirm="Apakah Anda yakin ingin menghapus PPPoE User ini?"
                                        ...
                                        <?php else: ?>
                                            <div class="py-1">
                                                <button
                                                    type="button"
                                                    wire:click="restore(<?php echo e($user->id); ?>)"
                                                    wire:loading.attr="disabled"
                                                    wire:confirm="Apakah Anda yakin ingin memulihkan PPPoE User ini?"
                        ...
                        <tr>
                            <td colspan="10" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-gray-900 mb-1">Belum ada PPPoE User</h3>
                                    <p class="text-gray-500 mb-6">Silakan tambahkan PPPoE User pertama.</p>
                                    <a href="<?php echo e(route('isp.pppoe-users.create')); ?>" class="inline-flex items-center gap-2 px-5 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors font-medium">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                        </svg>
                                        Tambah User
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($pppoeUsers->hasPages()): ?>
            <div class="bg-white px-6 py-4 border-t border-gray-200">
                <?php echo e($pppoeUsers->links()); ?>
            ...
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-xl shadow-xl sm:align-middle">
                    <div class="mb-4">
                        ...
                        <h3 class="text-lg font-semibold text-gray-900 mb-2 text-center">Hapus <?php echo e(count($selectedIds)); ?> PPPoE User?</h3>
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($showImportModal): ?>
        <div x-data="{ open: <?php if ((object) ('showImportModal') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('showImportModal'->value()); ?>')<?php echo e('showImportModal'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('showImportModal'); ?>')<?php endif; ?> }" x-show="open" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div class="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white rounded-xl shadow-xl sm:align-middle">
                    <div class="mb-4">
                        <h3 class="text-lg font-semibold text-gray-900 mb-2">Import PPPoE User</h3>
                        <p class="text-sm text-gray-500 mb-4">Upload file Excel (.xlsx, .xls) atau CSV untuk mengimport PPPoE User.</p>
                        <div class="mb-4">
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\isp\pppoe-user\index.blade.php ENDPATH**/ ?>