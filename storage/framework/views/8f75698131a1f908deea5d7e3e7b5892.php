<?php $__env->startSection('page_title'); ?>
    <span class="material-symbols-outlined notranslate text-indigo-500" translate="no" style="font-size:24px">share</span>
    <span class="text-lg">Data ODP</span>
...
<div class="space-y-5 pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('isp.odps.create')); ?>" class="inline-flex items-center justify-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-medium rounded-lg shadow-sm shadow-indigo-200 dark:shadow-none transition-all">
    ...
    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($selectedODPs ?? $selectedODP ?? $selectedOdcs ?? $selectedOdps ?? $selectedOlts)): ?>
    <div class="flex flex-wrap items-center gap-2 p-3 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 rounded-lg">
        <span class="text-sm text-indigo-800 dark:text-indigo-300 font-medium mr-2"><?php echo e(count($selectedOdps ?? $selectedOdcs ?? $selectedOdps ?? $selectedOlts)); ?> Terpilih</span>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-slate-50/80 dark:bg-slate-800/80">
                    <tr class="border-b border-slate-200 dark:border-slate-700 text-xs uppercase tracking-wider text-slate-500 dark:text-slate-400">
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Nama ODP</th>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">ODC</th>
                ...
                <tbody class="bg-white dark:bg-slate-800 divide-y divide-gray-200 dark:divide-gray-700">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_0 = true; $__currentLoopData = $odps; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $odp): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_0 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr <?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::$currentLoop['key'] = ''.e($odp->id).''; ?>wire:key="<?php echo e($odp->id); ?>" class="hover:bg-gray-50 dark:hover:bg-slate-700/50 dark:bg-slate-800 dark:hover:bg-slate-700/50 dark:bg-slate-800 transition-colors <?php if($odp->trashed()): ?> bg-red-50 dark:bg-red-900/30 <?php endif; ?>">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <input type="checkbox" wire:model.live="selectedOdps" value="<?php echo e($odp->id); ?>" class="w-4 h-4 text-primary-600 border-gray-300 dark:border-slate-600 rounded focus:ring-primary-500" <?php if($odp->trashed()): ?> disabled <?php endif; ?> />
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <?php else: ?>
                                    <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full <?php echo e($odp->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 dark:bg-slate-700 text-gray-800 dark:text-gray-200'); ?>">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-500 dark:text-slate-400 truncate max-w-[200px]" title="<?php echo e($odp->address); ?>"><?php echo e($odp->address ?: '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="font-mono text-sm text-gray-900 dark:text-slate-100"><?php echo e($odp->code); ?></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900 dark:text-slate-100"><?php echo e($odp->odc->name ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="<?php echo e(route('isp.odps.show', $odp->id)); ?>" class="p-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 rounded hover:bg-indigo-100 dark:bg-indigo-900/50 transition-colors" title="Detail">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:16px">visibility</span>
                                    </a>
                                    <a href="<?php echo e(route('isp.odps.edit', $odp->id)); ?>" class="p-1.5 bg-amber-50 dark:bg-amber-900/30 text-amber-600 rounded hover:bg-amber-100 dark:bg-amber-900/50 transition-colors" title="Edit">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:16px">edit</span>
                                    </a>
                                    <button type="button" wire:click="delete(<?php echo e($odp->id); ?>)" wire:confirm="Yakin ingin menghapus ODP ini?" class="p-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 rounded hover:bg-red-100 dark:bg-red-900/50 transition-colors" title="Hapus">
                        ...
                        <tr>
                            <td colspan="10" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <h3 class="text-lg font-semibold text-gray-900 dark:text-slate-100 mb-1">Belum ada ODP</h3>
                                    <p class="text-gray-500 dark:text-slate-400 mb-6">Silakan tambahkan ODP pertama.</p>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\isp\odp\index.blade.php ENDPATH**/ ?>