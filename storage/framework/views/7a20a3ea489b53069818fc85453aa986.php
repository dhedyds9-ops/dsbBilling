...
<?php endif; ?>
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <div class="flex items-center gap-3">
                <div class="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 text-2xl font-bold">
                    <?php echo e(substr($voucher->code, 0, 1)); ?>

                </div>
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($voucher->code); ?></h1>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="px-3 py-1 text-xs font-medium rounded-full 
                            <?php if($voucher->status === 'available'): ?> bg-green-100 dark:bg-green-900/50 text-green-600
                            <?php elseif($voucher->status === 'used'): ?> bg-blue-100 dark:bg-blue-900/50 text-blue-600
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('aaa.vouchers.edit', $voucher->id)); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-slate-900 dark:text-slate-100 font-mono"><?php echo e($voucher->code); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Voucher Pool</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($voucher->voucherPool?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Status</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e(ucfirst($voucher->status)); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Dibuat Oleh</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($voucher->createdBy?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tanggal Kadaluarsa</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($voucher->expires_at?->format('d/m/Y H:i') ?? '-'); ?></p>
                    </div>
                </div>
            </div>
         <?php echo $__env->renderComponent(); ?>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\aaa\voucher\show.blade.php ENDPATH**/ ?>