...
<head>
    ...
    <title><?php echo e(config('app.name', 'dsBilling')); ?> - Guest Payment</title>
...
<body class="font-sans text-on-surface antialiased min-h-screen py-10 px-4 sm:px-6 lg:px-8">
    <div class="max-w-3xl mx-auto relative z-10">
        ...
        <div class="text-center mb-12">
            <a href="<?php echo e(route('home')); ?>" class="inline-flex flex-col items-center gap-4">
                <?php
                ...
                <span class="text-3xl font-heading font-extrabold text-white tracking-tight drop-shadow-md"><?php echo e($companyName); ?></span>
            </a>
        ...
        <div class="bg-surface-container/60 backdrop-blur-2xl rounded-[24px] p-6 sm:p-10 mb-10 border border-primary/20 shadow-[0_0_25px_rgba(76,215,246,0.15)]">
            <form action="<?php echo e(route('guest.payment')); ?>" method="GET">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($query): ?>
            ...
            <?php else: ?>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($hotspot || $voucher): ?>
                    <div class="bg-surface-container/60 backdrop-blur-2xl rounded-[24px] p-6 sm:p-8 mb-8 border border-outline-variant shadow-xl">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div class="bg-surface-container-highest rounded-xl p-5 border border-outline-variant/50">
                                ...
                                <p class="text-lg font-bold text-white"><?php echo e($hotspot->username ?? $voucher->code); ?></p>
                            </div>
                            <div class="bg-surface-container-highest rounded-xl p-5 border border-outline-variant/50">
                                <p class="text-sm text-gray-400 mb-1">Paket</p>
                                <p class="text-lg font-bold text-white"><?php echo e($hotspot->serviceProfile->name ?? $voucher->serviceProfile->name ?? '-'); ?></p>
                            </div>
                            <div class="bg-surface-container-highest rounded-xl p-5 border border-outline-variant/50">
                                <p class="text-sm text-gray-400 mb-1">Status Sesi</p>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($activeSessions > 0): ?>
                            ...
                            <div class="bg-surface-container-highest rounded-xl p-5 border border-outline-variant/50">
                                ...
                                <p class="text-base font-bold text-white">
                                    <?php echo e($hotspot->uptime_limit ?? $voucher->uptime_limit ?? 'Unlimited'); ?>

                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($hotspot->bytes_out_limit || $voucher->bytes_out_limit): ?>
                                        / <?php echo e($hotspot->bytes_out_limit ?? $voucher->bytes_out_limit); ?>
                        ...
                        <div class="p-5 bg-error/10 border border-error/20 rounded-xl">
                            <div class="flex items-start gap-4">
                                ...
                                <div class="flex-1">
                                    ...
                                    <form method="POST" action="<?php echo e(route('guest.payment.reset')); ?>">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="hotspot_user_id" value="<?php echo e($hotspot->id); ?>">
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($customer && $invoices->isNotEmpty()): ?>
                    <div class="mb-6 flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <h2 class="text-xl font-heading font-bold text-white"><?php echo e($customer->name); ?></h2>
                        </div>
                    </div>
                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        ...
                        <div class="bg-surface-container/60 backdrop-blur-2xl rounded-[24px] p-8 flex flex-col sm:flex-row items-center justify-between gap-8 hover:bg-surface-container-highest transition-all duration-300 border border-primary/20 shadow-[0_0_25px_rgba(76,215,246,0.15)]">
                            <div class="flex-1 w-full text-center sm:text-left">
                                <div class="flex flex-col sm:flex-row items-center gap-4 mb-4">
                                    ...
                                    <span class="text-base font-bold text-white"><?php echo e($inv->invoice_number); ?></span>
                                </div>
                                <h3 class="text-4xl font-heading font-black text-white mb-3 tracking-tight">Rp <?php echo e(number_format($sisa, 0, ',', '.')); ?></h3>
                                <p class="text-base font-medium text-gray-400">
                                    Jatuh Tempo: <span class="<?php echo e(\Carbon\Carbon::parse($inv->due_date)->isPast() ? 'text-error font-bold' : 'text-white font-bold'); ?>"><?php echo e(\Carbon\Carbon::parse($inv->due_date)->translatedFormat('d F Y')); ?></span>
                                </p>
                            </div>
                            <div class="w-full sm:w-auto shrink-0 flex flex-col items-center">
                                <button type="button" onclick="openPaymentModal('<?php echo e($inv->invoice_number); ?>', <?php echo e($sisa); ?>)" class="w-full sm:w-auto px-6 py-4 sm:px-8 sm:py-5 bg-gradient-to-r from-primary-container to-secondary-container hover:from-primary hover:to-secondary-container text-white font-heading font-bold rounded-xl shadow-lg shadow-primary-container/30 transition-all active:scale-95 flex items-center justify-center gap-2 sm:gap-3 text-base sm:text-lg whitespace-nowrap">
        ...
        <div class="text-center mt-16 mb-12 mt-4">
            <a href="<?php echo e(route('home')); ?>" class="inline-flex items-center gap-2 px-6 py-3 rounded-full bg-surface-container-highest hover:bg-surface-bright backdrop-blur-md text-sm font-bold text-white transition-all hover:scale-105 border border-outline-variant shadow-[0_0_15px_rgba(76,215,246,0.1)]">
    ...
    <div id="paymentModal" class="fixed inset-0 z-50 flex items-center justify-center px-4" style="display: none;">
        ...
        <div class="bg-surface-container rounded-[24px] shadow-2xl w-full max-w-md relative z-10 overflow-hidden flex flex-col transform transition-all border border-outline-variant">
            ...
            <div class="p-6">
                ...
                <form method="POST" action="<?php echo e(route('guest.payment.checkout')); ?>">
                    <?php echo csrf_field(); ?>
                    ...
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($activeGateways)): ?>
                        <div class="space-y-3 mb-8 max-h-[40vh] overflow-y-auto pr-2 custom-scrollbar">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $activeGateways; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $g): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                            <label class="flex items-center justify-between p-4 border border-outline-variant rounded-xl cursor-pointer hover:bg-surface-container-highest transition-all group has-[:checked]:border-primary has-[:checked]:bg-primary/10 has-[:checked]:ring-1 has-[:checked]:ring-primary">
                                <div class="flex items-center gap-3">
                                    <input type="radio" name="gateway" value="<?php echo e($key); ?>" class="text-primary focus:ring-primary border-outline-variant bg-surface-container-highest w-5 h-5 dark:bg-slate-900 dark:text-slate-100" <?php echo e($loop->first ? 'checked' : ''); ?>>
                                    <span class="font-bold text-white group-has-[:checked]:text-primary"><?php echo e(strtoupper($key)); ?></span>
                                </div>
    ...
    <script>
        ...
        function handlePayManual() {
            ...
            const phone = "<?php echo e(preg_replace('/[^0-9]/', '', \App\Models\Setting::getValue('company.phone', '081234567890'))); ?>";
...
<?php /**PATH D:\dsBilling\resources\views/guest/payment.blade.php ENDPATH**/ ?>