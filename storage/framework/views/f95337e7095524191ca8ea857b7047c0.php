<div class="space-y-6">
    <div class="flex items-center gap-4">
        <a href="<?php echo e(route('isp.hotspot-users.index')); ?>" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
        ...
        <div class="flex-1">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($hotspotUser->username); ?></h1>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="px-3 py-1 text-xs font-medium rounded-full 
                            <?php if($hotspotUser->status === 'active'): ?> bg-green-100 dark:bg-green-900/50 text-green-600
                            <?php elseif($hotspotUser->status === 'inactive'): ?> bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400
                            <?php elseif($hotspotUser->status === 'suspended'): ?> bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('isp.hotspot-users.edit', $hotspotUser->id)); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-slate-900 dark:text-slate-100 font-mono"><?php echo e($hotspotUser->username); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Service Profile</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($hotspotUser->serviceProfile?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Status</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e(ucfirst($hotspotUser->status)); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Dibuat Oleh</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($hotspotUser->createdBy?->name ?? '-'); ?></p>
                    </div>
                </div>
            </div>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\isp\hotspot-user\show.blade.php ENDPATH**/ ?>