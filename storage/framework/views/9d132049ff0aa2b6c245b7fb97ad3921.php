<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">Kelola semua quotation penawaran harga</p>
        </div>
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-slate-50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">Total Harga</th>
                ...
                <tbody class="divide-y divide-slate-200">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $quotations; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $quotation): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50">
                            <td class="px-6 py-4">
                                <div class="font-medium text-slate-900"><?php echo e($quotation->customer_name ?? '-'); ?></div>
                            </td>
                            <td class="px-6 py-4 text-sm text-slate-600">Rp <?php echo e(number_format($quotation->total ?? 0, 0, ',', '.')); ?></td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 text-xs font-medium rounded-full 
                                    <?php if($quotation->status === 'draft'): ?> bg-slate-100 text-slate-600
                                    <?php elseif($quotation->status === 'sent'): ?> bg-blue-100 text-blue-600
                                    <?php elseif($quotation->status === 'accepted'): ?> bg-green-100 text-green-600
                            ...
                            <td class="px-6 py-4 text-sm text-slate-600"><?php echo e($quotation->created_at?->format('d/m/Y')); ?></td>
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="<?php echo e(route('crm.quotations.show', $quotation->id)); ?>" class="p-2 text-slate-500 hover:text-primary-600 rounded-lg hover:bg-slate-100">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                        </svg>
                                    </a>
                                    <a href="<?php echo e(route('crm.quotations.edit', $quotation->id)); ?>" class="p-2 text-slate-500 hover:text-blue-600 rounded-lg hover:bg-slate-100">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
                                        </svg>
                                    </a>
                                    <button wire:click="delete(<?php echo e($quotation->id); ?>)" wire:confirm="Yakin ingin menghapus quotation ini?" class="p-2 text-slate-500 hover:text-red-600 rounded-lg hover:bg-slate-100">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\crm\quotation\index.blade.php ENDPATH**/ ?>