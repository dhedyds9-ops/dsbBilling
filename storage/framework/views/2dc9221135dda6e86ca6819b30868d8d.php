<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="<?php echo e(route('crm.contracts.index')); ?>" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
            </svg>
        </a>
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($contract->customer_name); ?></h1>
            <div class="flex items-center gap-2 mt-1">
                ...
                <span class="text-sm text-slate-500 dark:text-slate-400"><?php echo e($contract->start_date?->format('d/m/Y')); ?> - <?php echo e($contract->end_date?->format('d/m/Y')); ?></span>
            </div>
        </div>
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('crm.contracts.edit', $contract->id)); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                Edit
            </a>
        </div>
    </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
                 ...
                 <?php $__env->slot('header', null, []); ?> 
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Informasi Contract</h3>
                 <?php $__env->endSlot(); ?>
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Nama Pelanggan</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($contract->customer_name); ?></p>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tanggal Mulai</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($contract->start_date?->format('d/m/Y') ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tanggal Selesai</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($contract->end_date?->format('d/m/Y') ?? '-'); ?></p>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Catatan</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($contract->notes ?? '-'); ?></p>
                    </div>
                </div>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\crm\contract\show.blade.php ENDPATH**/ ?>