    ...
    <div class="mb-4 px-4 sm:px-0">
        ...
        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(session()->has('error')): ?>
            <div class="p-4 mb-4 text-sm text-rose-800 rounded-lg bg-rose-50 dark:bg-rose-900/30 dark:text-rose-400 border border-rose-200 dark:border-rose-800" role="alert">
                <span class="font-medium">Gagal!</span> <?php echo e(session('error')); ?>
  ...
  <div class="space-y-5 pb-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($search || count(array_filter($filters)) > 0): ?>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50 text-slate-700 dark:text-slate-300">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $devices; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $device): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <tr class="hover:bg-slate-50 dark:bg-slate-900/50/50 dark:hover:bg-slate-800/50 transition-colors">
                            <td class="px-4 py-3">
                                <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] font-medium uppercase tracking-wider <?php echo e($device->status === 'online' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400' : 'bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-400'); ?>">
                            ...
                            <td class="px-4 py-3 font-medium text-slate-900 dark:text-slate-100"><?php echo e($device->serial_number); ?></td>
                            <td class="px-4 py-3 font-mono text-xs"><?php echo e($device->mac_address ?? '-'); ?></td>
                            <td class="px-4 py-3">
                                <div class="font-medium text-slate-900 dark:text-slate-100"><?php echo e($device->model ?? '-'); ?></div>
                                <div class="text-[11px] text-slate-500 dark:text-slate-400"><?php echo e($device->vendor->name ?? $device->vendor ?? '-'); ?></div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="font-mono text-xs text-slate-900 dark:text-slate-100" title="PPPoE IP"><?php echo e($device->ip_address ?? '-'); ?></div>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($device->connection_request_url): ?>
                                    <?php
                                        $tr069Ip = parse_url($device->connection_request_url, PHP_URL_HOST);
                                    ?>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($tr069Ip && $tr069Ip !== $device->ip_address): ?>
                                        <div class="font-mono text-[11px] text-slate-500 dark:text-slate-400" title="TR069 IP (Connection Request)"><?php echo e($tr069Ip); ?></div>
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </td>
                            <td class="px-4 py-3"><?php echo e($device->software_version ?? '-'); ?></td>
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-1">
                                    <a href="<?php echo e(route('acs.devices.show', $device->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 transition-colors" title="Detail">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:18px">info</span>
                                    </a>
                                    <a href="<?php echo e(route('acs.devices.edit', $device->id)); ?>" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-900/30 dark:hover:bg-emerald-900/50 text-emerald-600 dark:text-emerald-400 transition-colors" title="Edit">
                                        <span class="material-symbols-outlined notranslate" translate="no" style="font-size:18px">edit</span>
                                    </a>
                                    <button wire:click="delete(<?php echo e($device->id); ?>)" wire:confirm="Yakin ingin menghapus device ini?" class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-rose-50 hover:bg-rose-100 dark:bg-rose-900/30 dark:hover:bg-rose-900/50 text-rose-600 dark:text-rose-400 transition-colors" title="Hapus">
...
<?php /**PATH D:\dsBilling\resources\views\livewire\acs\device\index.blade.php ENDPATH**/ ?>