    ...
    <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900"><?php echo e($voucher->code); ?></h1>
                    <div class="flex items-center gap-2 mt-1">
                        <span class="px-3 py-1 text-xs font-medium rounded-full 
                            <?php if($voucher->status === 'available'): ?> bg-green-100 text-green-600
                            <?php elseif($voucher->status === 'used'): ?> bg-blue-100 text-blue-600
        ...
        <div class="flex items-center gap-2">
            <a href="<?php echo e(route('isp.vouchers.edit', $voucher->id)); ?>" class="px-4 py-2 bg-white border border-slate-200 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors">
                Edit
            </a>
        </div>
    </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
             ...
             <?php $__env->slot('header', null, []); ?> 
                <h3 class="text-lg font-semibold text-slate-900">Informasi Voucher</h3>
             <?php $__env->endSlot(); ?>
            <div class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Kode</label>
                        <p class="text-slate-900 font-mono"><?php echo e($voucher->code); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Voucher Pool</label>
                        <p class="text-slate-900"><?php echo e($voucher->voucherPool?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Status</label>
                        <p class="text-slate-900"><?php echo e(ucfirst($voucher->status)); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Dibuat Oleh</label>
                        <p class="text-slate-900"><?php echo e($voucher->createdBy?->name ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 mb-1">Tanggal Kadaluarsa</label>
                        <p class="text-slate-900"><?php echo e($voucher->expires_at?->format('d/m/Y H:i') ?? '-'); ?></p>
                    </div>
                </div>
            </div>
...
<?php /**PATH C:\Users\Lenovo\OneDrive\dsBilling\resources\views\livewire\isp\voucher\show.blade.php ENDPATH**/ ?>