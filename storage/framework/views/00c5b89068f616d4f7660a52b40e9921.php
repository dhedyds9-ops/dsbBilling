...
<div class="h-full flex flex-col bg-slate-50 dark:bg-slate-900">
    ...
    <div class="flex-none px-3 py-2 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between gap-3 flex-wrap bg-white dark:bg-slate-800">
        <div class="flex items-center gap-3">
            ...
            <span class="px-2 py-1 rounded bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-300 text-xs font-semibold"><?php echo e($summary['total']); ?> Total</span>
            <span class="px-2 py-1 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-xs font-semibold"><?php echo e($summary['online']); ?> Online</span>
            <span class="px-2 py-1 rounded bg-slate-200 text-slate-700 dark:bg-slate-700 dark:text-slate-400 text-xs font-semibold"><?php echo e($summary['offline']); ?> Offline</span>
            <span class="px-2 py-1 rounded bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 text-xs font-semibold"><?php echo e($summary['los']); ?> LOS</span>
            <span class="px-2 py-1 rounded bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 text-xs font-semibold"><?php echo e($summary['low_rx']); ?> Low RX</span>
        </div>
        <div class="flex items-center gap-2">
            <select wire:model.live="oltFilter" class="bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-600 px-2 py-1 text-xs rounded border focus:outline-none dark:bg-slate-900 dark:text-slate-100">
                ...
                <option class="bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200" value="<?php echo e($olt->id); ?>"><?php echo e($olt->name); ?></option>
        ...
        <table class="w-full text-left text-sm text-slate-600 dark:text-slate-300 whitespace-nowrap">
            <thead class="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-700">
                <tr class="text-slate-500 dark:text-slate-400">
                    <th class="p-3 font-semibold" wire:click="sort('serial_number')">Serial <?php echo $sortField === 'serial_number' ? ($sortDirection === 'asc' ? '&uarr;' : '&darr;') : ''; ?></th>
                    ...
                    <th class="p-3 font-semibold" wire:click="sort('rx_power_dbm')">RX Power <?php echo $sortField === 'rx_power_dbm' ? ($sortDirection === 'asc' ? '&uarr;' : '&darr;') : ''; ?></th>
                    ...
                    <th class="p-3 font-semibold" wire:click="sort('last_seen_at')">Last Seen <?php echo $sortField === 'last_seen_at' ? ($sortDirection === 'asc' ? '&uarr;' : '&darr;') : ''; ?></th>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                ...
                <tr class="hover:bg-slate-50 dark:bg-slate-900/50 dark:hover:bg-slate-800/50 transition-colors ">
                    <td class="p-3 font-mono text-slate-800 dark:text-slate-200 font-medium"><?php echo e($onu->serial_number); ?></td>
                    <td class="p-3 text-slate-600 dark:text-slate-300"><?php echo e($onu->name ?? '-'); ?></td>
                    <td class="p-3 text-slate-500 dark:text-slate-400">
                        <span class="text-slate-500 dark:text-slate-400"><?php echo e($onu->olt->name ?? '-'); ?></span>
                        <span class="text-slate-500 dark:text-slate-400 opacity-60">/</span>
                        <span class="font-mono text-slate-500 dark:text-slate-400"><?php echo e($onu->ponPort->name ?? '-'); ?></span>
                    </td>
                    <td class="p-3 text-slate-500 dark:text-slate-400"><?php echo e($onu->customerService->customer->name ?? '-'); ?></td>
                    <td class="p-3 font-mono font-medium <?php echo e($rxClass); ?>"><?php echo e($onu->rx_power_dbm !== null ? number_format($onu->rx_power_dbm-1) . ' dBm' : '-'); ?></td>
                    <td class="p-3 font-mono text-slate-500 dark:text-slate-400"><?php echo e($onu->tx_power_dbm !== null ? number_format($onu->tx_power_dbm-1) . ' dBm' : '-'); ?></td>
                    <td class="p-3 font-mono text-slate-500 dark:text-slate-400"><?php echo e($onu->temperature ?? '-'); ?></td>
                    <td class="p-3">
                        <span class="px-1.5 py-0.5 rounded text-xs font-medium <?php echo e($badge); ?>"><?php echo e($st); ?></span>
                    </td>
                    <td class="p-3 text-slate-500 dark:text-slate-400"><?php echo e($onu->last_seen_at ? $onu->last_seen_at->diffForHumans() : '-'); ?></td>
                    <td class="p-3 flex items-center gap-3">
                        <a href="<?php echo e(route('isp.onus.show', $onu->id)); ?>" class="text-blue-500 hover:text-blue-400 text-sm font-medium">View</a>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($st === 'OFFLINE'): ?>
                        <button type="button" wire:click="deleteOnu(<?php echo e($onu->id); ?>)" wire:confirm="Yakin ingin menghapus ONU ini dari database? (Ini tidak akan menghapus ONU dari OLT)" class="text-red-500 hover:text-red-400 text-sm font-medium">
                            Hapus
                        </button>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\isp\onu\index-v2.blade.php ENDPATH**/ ?>