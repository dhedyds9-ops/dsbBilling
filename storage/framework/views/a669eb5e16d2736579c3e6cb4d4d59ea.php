...
<?php endif; ?>
        <div class="flex items-center gap-4">
            <a href="<?php echo e(route('isp.pops.index')); ?>" class="p-2 text-slate-500 hover:text-slate-700 dark:text-slate-300 rounded-lg hover:bg-slate-100 dark:bg-slate-800">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3">
                    <div class="w-14 h-14 rounded-full bg-primary-100 flex items-center justify-center text-primary-600 text-2xl font-bold">
                        <?php echo e(substr($pop->name, 0, 1)); ?>

                    </div>
                    <div>
                        <h1 class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($pop->name); ?></h1>
                        <div class="flex items-center gap-2 mt-1">
                            <span class="px-3 py-1 text-xs font-medium rounded-full 
                                <?php if($pop->status === 'active'): ?> bg-green-100 dark:bg-green-900/50 text-green-600
                                <?php else: ?> bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400
                                <?php endif; ?>">
                                <?php echo e(ucfirst($pop->status)); ?>

                            </span>
                            <span class="text-sm text-slate-500 dark:text-slate-400 font-mono"><?php echo e($pop->code); ?></span>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex items-center gap-2">
                <a href="<?php echo e(route('isp.pops.edit', $pop->id)); ?>" class="px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-lg text-sm font-medium hover:bg-slate-50 dark:bg-slate-900/50 transition-colors">
                    Edit
                </a>
            </div>
        </div>
...
<?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::processComponentKey($component); ?>
                 ...
                 <?php $__env->slot('header', null, []); ?> 
                    <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Informasi POP</h3>
                 <?php $__env->endSlot(); ?>
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Kode</label>
                            <p class="text-slate-900 dark:text-slate-100 font-mono"><?php echo e($pop->code); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Nama</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->name); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Tower</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->tower->name ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Provinsi</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->province ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Kota</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->city ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Kecamatan</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->district ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Kelurahan</label>
                            <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->village ?? '-'); ?></p>
                        </div>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Alamat</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->address ?? '-'); ?></p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Deskripsi</label>
                        <p class="text-slate-900 dark:text-slate-100"><?php echo e($pop->description ?? '-'); ?></p>
                    </div>
                </div>
             <?php echo $__env->renderComponent(); ?>
<?php endif; ?>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="text-slate-900 dark:text-slate-100 font-mono"><?php echo e($pop->latitude ?? '-'); ?></p>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">Longitude</label>
                            <p class="text-slate-900 dark:text-slate-100 font-mono"><?php echo e($pop->longitude ?? '-'); ?></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 pt-4 border-t border-slate-200 dark:border-slate-700">
                        <div class="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                            ...
                            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($pop->olts->count()); ?></p>
                        </div>
                        <div class="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg">
                            <p class="text-sm text-slate-500 dark:text-slate-400">ODC</p>
                            <p class="text-2xl font-bold text-slate-900 dark:text-slate-100"><?php echo e($pop->odcs->count()); ?></p>
                        </div>
                    </div>
                </div>
             <?php echo $__env->renderComponent(); ?>
...
<?php /**PATH D:\dsBilling\resources\views\livewire\isp\pop\show.blade.php ENDPATH**/ ?>