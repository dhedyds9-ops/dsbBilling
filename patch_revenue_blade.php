...
$tdSearch = <<<PHP
                            <td class="px-6 py-3">
                                @if(\$row->invoice && \$row->invoice->contract_id)
...
$tdReplace = <<<PHP
                            ...
                            <td class="px-6 py-3">
                                @php
                                    \$sp = (\$row->invoice && \$row->invoice->customer) ? \$row->invoice->customer->customerServices()->with('serviceProfile')->first() : null;
                                @endphp
